...
class WarpApi {
  ...
  /// Non-blocking sync that returns immediately. Progress and completion
  /// are reported via the port. Completion is signaled by height=0xFFFFFFFF.
  static void warpSyncAsync(int coin, int account, bool getTx,
      int anchorOffset, int maxCost, int port) {
    warp_api_lib.warp_async(
        coin, account, getTx ? 1 : 0, anchorOffset, maxCost, port);
  }
  ...
  /// Check if an account index has any on-chain activity (transparent UTXOs or transactions)
  /// Returns true if activity detected, false otherwise.
  /// Used for account discovery during seed restoration.
  static Future<bool> checkAccountActivity(int coin, String seed, int accountIndex) async {
    return await compute((_) {
      final seedPtr = toNative(seed);
      try {
        final result = warp_api_lib.check_account_activity(coin, seedPtr, accountIndex);
        final activityValue = unwrapResultU8(result);
  ...
  /// Scan for shielded accounts using ZIP-32 derivation with trial decryption.
  /// Returns a list of account indexes that have shielded activity.
  /// This properly detects accounts by checking if viewing keys can decrypt notes.
  /// 
  /// Parameters:
  /// DEBUG: Derive FVK for a seed at a specific index (for testing key derivation)
  static String deriveFvkForIndex(int coin, String seed, int index) {
    final seedPtr = toNative(seed);
    try {
      final result = warp_api_lib.derive_fvk_for_index(coin, seedPtr, index);
  ...
  /// - seed: seed phrase
  /// - maxIndex: maximum index to scan (0-9)
  /// - numBlocks: number of recent blocks to scan (1000-5000 recommended)
  static Future<List<int>> scanShieldedAccounts(int coin, String seed, {int maxIndex = 9, int numBlocks = 2500}) async {
    print('[DART-SCAN] scanShieldedAccounts called: coin=$coin, maxIndex=$maxIndex, numBlocks=$numBlocks');
    print('[DART-SCAN] Seed word count: ${seed.split(' ').length}');
    
    // DEBUG: Derive and print FVK for index 0 so we can compare with database
    try {
      final fvk0 = deriveFvkForIndex(coin, seed, 0);
      print('[DART-SCAN] Derived FVK for index 0: ${fvk0.substring(0, 80)}...');
    } catch (e) {
      print('[DART-SCAN] Error deriving FVK: $e');
    }
    
    // Run in compute isolate
    final bitmask = await compute((Map<String, dynamic> args) {
      final seed = args['seed'] as String;
      final coin = args['coin'] as int;
      final maxIndex = args['maxIndex'] as int;
      final numBlocks = args['numBlocks'] as int;
      
      final seedPtr = toNative(seed);
      try {
        final result = warp_api_lib.scan_shielded_accounts(coin, seedPtr, maxIndex, numBlocks);
        return unwrapResultU32(result);
      } finally {
        malloc.free(seedPtr);
      }
    }, {'seed': seed, 'coin': coin, 'maxIndex': maxIndex, 'numBlocks': numBlocks});
    
    print('[DART-SCAN] Raw bitmask result: $bitmask (binary: ${bitmask.toRadixString(2)})');
    
    // Convert bitmask to list of active indexes
    List<int> activeIndexes = [];
    for (int i = 0; i <= maxIndex; i++) {
      if ((bitmask & (1 << i)) != 0) {
        activeIndexes.add(i);
        print('[DART-SCAN] Found activity at index $i');
      }
    }
    
    print('[DART-SCAN] scanShieldedAccounts result: $activeIndexes');
