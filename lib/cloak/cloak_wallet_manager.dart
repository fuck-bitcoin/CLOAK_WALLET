...
class CloakWalletManager {
  ...
  static Future<Map<String, dynamic>> generateShieldEsrSimple({
    ...
    if (Platform.isAndroid) {
      ...
      // Anchor Android doesn't support variant 2 ESR or the WebSocket relay
      // backchannel. Instead, we use:
      //   Step 1: ESR with ONLY user's 2 transfer actions + flags=3 (broadcast
      //           + background POST callback to Buoy relay). Anchor broadcasts
      //           the transfers and POSTs the result to cb.anchor.link/{uuid}.
      //   Step 2: After receiving the callback, wallet broadcasts begin/mint/end
      //           separately via broadcastMintOnly() (thezeosalias-only actions).
      //
      // The callback UUID is generated by the EsrDisplayDialog's AnchorLinkClient
      // and passed via the 'androidCallbackUrl' field in shieldData.
      // The ESR URL will be regenerated in the dialog once the channel is ready.
      esrUrl = ''; // Placeholder — regenerated in EsrDisplayDialog with callback
    } else {
