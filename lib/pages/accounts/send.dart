...
class SendContext {
  ...
  SendContext(this.address, this.pools, this.amount, this.memo,
      ...
      this.nftImageUrl,
      this.nftName,
      this.isBatchWithdraw = false,
      this.batchAssets,
      this.isVaultDeposit = false,
      this.precomputedFeeUnits,
      this.isDrainSend = false]);
...
class BatchAsset {
  ...
  String get formattedAmount =>
      '${formatAssetUnits(amountUnits, precision)} $symbol';
}
...
String? _getSendTokenLogoUrl(String symbol, String contract) {
  const wellKnown = {
    ...
    'eosio.token:TLOS':
        'https://raw.githubusercontent.com/AnyswapIN/nftlist/main/telos.png',
  };
...
List<_SendToken> _parseShieldedTokens() {
  final tokens = parsePositiveShieldedFtBalances(
    CloakWalletManager.getBalancesJson(),
  )
      .map((balance) => _SendToken(
...
class _SendNft {
  ...
  _SendNft(
      {required this.nftId, required this.contract, this.name, this.imageUrl});
}
...
List<_SendToken> _parseVaultTokens() {
  ...
  for (final ft in vt.fts) {
    ...
    final balanceUnits = rawAmount is int ? rawAmount : parsed?.units;
    if (parsed == null || balanceUnits == null || balanceUnits <= 0) continue;
...
class _QuickSendState extends State<QuickSendPage> with WithLoadingAnimation {
  ...
  static PoolBalanceT _getWalletBalanceFromRust() {
    ...
    for (final balance in parsePositiveShieldedFtBalances(raw)) {
      if (balance.symbol == 'CLOAK' && balance.contract == 'thezeostoken') {
        bal.sapling = balance.units;
        break;
      }
    }
    return bal;
  ...
  int?
      _sendMaxFeeUnits; // Precomputed fee from Send Max (passed to confirm page)
  bool _isDrainSend = false; // Drain mode: Rust selects ALL notes for send max
  MemoData _memo = MemoData(appSettings.includeReplyTo != 0, '', '');
  ...
  Widget build(BuildContext context) {
    ...
    final bool _isCloakSelected =
        (_selectedToken?.symbol ?? 'CLOAK') == 'CLOAK' &&
            (_selectedToken?.contract ?? 'thezeostoken') == 'thezeostoken';
    ...
    final chipBgColor =
        Color.lerp(addressFillColor, Colors.black, 0.06) ?? addressFillColor;
    final chipBorderColor = (t.extension<ZashiThemeExt>()?.quickBorderColor) ??
        t.dividerColor.withOpacity(0.20);
    // Cursor color aligned with the ZEC balance text color
    final balanceCursorColor =
        t.extension<ZashiThemeExt>()?.balanceAmountColor ??
                  ...
                  child: Column(
                    children: [
                      BalanceWidget(
                        0,
                        balanceOverride:
                            (widget.sendContext?.isVaultDeposit == true)
                                ? balances.sapling
                                : null,
                      ),
                      Gap(24),
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  TextField(
                                    ...
                                    readOnly: (widget.sendContext?.fromThread ??
                                            false) ||
                                        (widget.sendContext?.isVaultDeposit ==
                                            true),
                                    ...
                                    decoration: InputDecoration(
                                      hintText: CloakWalletManager.isCloak(
                                              aa.coin)
                                          ? (_isVaultWithdrawMode
                                              ? 'CLOAK or Telos Address'
                                              : 'Address, Account, or Vault Hash')
                                          : 'Zcash Address',
                                      ...
                                      suffixIcon: (widget.sendContext
                                                  ?.isVaultDeposit ==
                                              true)
                                          ? null
                                          : GestureDetector(
                                              onTap: () async {
                                                if ((widget.sendContext
                                                        ?.fromThread ??
                                                    false)) {
                                                  return;
                                                }
                                  ...
                                  if (!_isNftMode) ...[
                                    ...
                                    ZashiAmountRow(
                                      ...
                                      tokenSymbol:
                                          _selectedToken?.symbol ?? 'CLOAK',
                                      showFiat: _isCloakSelected,
                                      tokenPrecision:
                                          _selectedToken?.precision ?? 4,
                                      onAmountChanged: (int value) {
                                        setState(() {
                                          _amount =
                                              Amount(value, _amount.deductFee);
                                          _sendMaxFeeUnits =
                                              null; // Clear precomputed fee on manual edit
                                          _isDrainSend = false;
                                          if (value > 0 && (_address.isEmpty)) {
                                            _addressError = CloakWalletManager
                                                    .isCloak(aa.coin)
                                                ? 'Enter address, account, or vault hash'
                                                : 'Enter Zcash Address';
                                          } else if (value == 0 &&
                                              _address.isEmpty) {
                                            _addressError = null;
                                      ...
                                      onSendMax: _isCloakSelected
                                          ? () async {
                                              final result =
                                                  await CloakWalletManager
                                                      .getMaxSendable(
                                                recipientAddress:
                                                    _address.isNotEmpty
                                                        ? _address
                      ...
                      if (_addressIsValid &&
                          (_isNftMode ||
                              (_amount.value > 0 &&
                                  _amount.value <= spendable)) &&
                          (CloakWalletManager.isCloak(aa.coin) ||
                              appStore.proverReady))
                        Align(
                          alignment: Alignment.center,
                          child: FractionallySizedBox(
                            widthFactor: 0.96,
  ...
  send() async {
    ...
    if (form.validate()) {
      ...
      final sc = SendContext(
        ...
        prev?.threadIndex,
        prev?.threadCid,
        _isNftMode ? null : _selectedToken?.symbol,
        _isNftMode ? null : _selectedToken?.contract,
        _isNftMode ? null : _selectedToken?.precision,
      ...
      try {
        ...
        if (scExtra?.fromThread == true) {
          ...
          if (cid == null || cid.isEmpty) {
            try {
              cid = await CloakDb.getProperty(
                      'contact_cid_' + aa.id.toString()) ??
                  '';
            } catch (_) {}
          }
          if (cid != null && cid.isNotEmpty) {
            ...
            try {
              final s0 =
                  (await CloakDb.getProperty('cid_my_seq_' + cid) ?? '').trim();
            ...
            try {
              await CloakDb.setProperty('cid_my_seq_' + cid, mySeq.toString());
            } catch (_) {}
            final amt = _amount.value;
            String header = 'v1; type=payment; conversation_id=' +
                cid +
                '; seq=' +
                mySeq.toString() +
                (amt > 0 ? '; amount_zat=' + amt.toString() : '');
            final bodyOnly = (effectiveMemo.memo).trim();
            effectiveMemo = MemoData(effectiveMemo.reply, effectiveMemo.subject,
                header + '\n\n' + bodyOnly);
          }
        }
      } catch (_) {}
      ...
      if (CloakWalletManager.isCloak(aa.coin)) {
        if (widget.single) {
          ...
          GoRouter.of(context)
              .pop({'address': _address, 'amount': _amount.value});
        }
      } else {
        // Non-CLOAK transaction flow no longer supported
        showMessageBox2(context, S.of(context).error,
            'Only CLOAK transactions are supported');
      }
    }
  }
  ...
  void _didUpdateSendContext(SendContext? sendContext) {
    ...
    _memo =
        sendContext.memo ?? MemoData(appSettings.includeReplyTo != 0, '', '');
    ...
    if (sendContext.fromThread == true) {
      ...
      final headerFixed =
          'v1; type=payment; conversation_id=${cid}; '.length + '\n\n'.length;
    ...
    if (sendContext.tokenSymbol != null && sendContext.tokenContract != null) {
      final matchIdx = _shieldedTokens.indexWhere(
        (t) =>
            t.symbol == sendContext.tokenSymbol &&
            t.contract == sendContext.tokenContract,
      );
      if (matchIdx >= 0) {
        setState(() {
    ...
    if (sendContext.nftId != null && sendContext.nftContract != null) {
      final matchIdx = _shieldedNfts.indexWhere(
        (n) =>
            n.nftId == sendContext.nftId &&
            n.contract == sendContext.nftContract,
      );
      setState(() {
        if (matchIdx >= 0) {
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (ctx, setSheetState) {
            ...
            Widget buildTokensList() {
              ...
              return Column(
                ...
                children: List.generate(_shieldedTokens.length, (i) {
                  ...
                  final isSelected = !_isNftMode &&
                      _selectedToken?.symbol == token.symbol &&
                      _selectedToken?.contract == token.contract;
                  return Material(
                    color: isSelected
            ...
            // ── NFT card builder ──
            Widget _buildSendNftCard(
                BuildContext ctx, ThemeData t, dynamic nft) {
              final isSelected = _isNftMode &&
                  _selectedNft?.nftId == nft.nftId &&
                  _selectedNft?.contract == nft.contract;
              return AspectRatio(
                aspectRatio: 1.0,
                    ...
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: Stack(
                          ...
                          children: [
                            NftImageWidget(
                                imageUrl: nft.imageUrl, assetId: nft.nftId),
                            if (isSelected)
                              Positioned(
                                top: 6,
                                right: 6,
                                ...
                                AnimatedPositioned(
                                  ...
                                  child: Container(
                                    decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.circular(999),
                                      border:
                                          Border.all(color: indicatorBorder),
                                    ),
                                  ),
                                ),
                                // Labels
...
class ZashiAmountRow extends StatefulWidget {
  ...
  const ZashiAmountRow(
      {super.key,
      required this.initialAmount,
      required this.fiatCode,
      required this.availableZatoshis,
      required this.onAmountChanged,
      this.tokenSymbol = 'CLOAK',
      this.showFiat = true,
      this.tokenPrecision = 4,
      this.onSendMax});
...
class _ZashiAmountRowState extends State<ZashiAmountRow> {
  ...
  late final NumberFormat _fiatFmt = NumberFormat.decimalPatternDigits(
  ...
  String _formatFiatDynamic(double x) {
    ...
    return NumberFormat.decimalPatternDigits(
            locale: Platform.localeName, decimalDigits: digits)
        .format(x);
  }

  bool _updating = false;
  bool _userEditing =
      false; // true while user is typing → skip didUpdateWidget reformat
  bool _syncing = false;
  ...
  void initState() {
    ...
    _unitScale = assetUnitScale(_currentTokenPrecision);
    if (widget.initialAmount > 0) {
      _zecCtl.text = formatAssetUnits(
        widget.initialAmount,
        _currentTokenPrecision,
      );
    } else {
      _zecCtl.text = '';
    }
  ...
  void didUpdateWidget(ZashiAmountRow oldWidget) {
    ...
    if (widget.tokenSymbol != _currentTokenSymbol ||
        widget.tokenPrecision != _currentTokenPrecision) {
      _currentTokenSymbol = widget.tokenSymbol;
      _currentTokenPrecision = _isCloak ? widget.tokenPrecision : 8;
      _unitScale = assetUnitScale(_currentTokenPrecision);
      _zecCtl.text = '';
  ...
  int? _parseCryptoUnits(String raw) {
    ...
    return parseAssetUnitsAtPrecision(normalized, _currentTokenPrecision);
  }
  ...
  void _syncFiatFromZec() {
    ...
    try {
      ...
      if (raw.isEmpty) {
        ...
        final units = _parseCryptoUnits(raw);
        if (units == null) throw const FormatException('Invalid amount');
        final z = units / _unitScale;
        final fiat = z * _fxRate!;
        _fiatCtl.text =
            _isCloak ? _formatFiatDynamic(fiat) : _fiatFmt.format(fiat);
        _insufficient = units > widget.availableZatoshis;
      }
    } catch (_) {
      _fiatCtl.text = '';
      _insufficient = false;
  ...
  void _syncZecFromFiat() {
    ...
    try {
      ...
      if (raw.isEmpty) {
        ...
        _zecCtl.text = formatAssetUnits(valueZats, _currentTokenPrecision);
  ...
  Widget build(BuildContext context) {
    ...
    final balanceFontFamily = t.textTheme.displaySmall?.fontFamily;
    final balanceTextColor = t.extension<ZashiThemeExt>()?.balanceAmountColor ??
        const Color(0xFFBDBDBD);
    ...
    final cryptoField = Theme(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          errorStyle: (t.textTheme.bodySmall ?? const TextStyle())
        ...
        onChanged: (_) {
          ...
          final value = _parseCryptoUnits(txt) ?? 0;
                    ...
                    children: [
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          errorText:
                              _insufficient ? 'Insufficient funds' : null,
                          errorStyle: (t.textTheme.bodySmall ??
                                  const TextStyle())
                              .copyWith(
                                  color: Theme.of(context).colorScheme.error),
                        ),
                        onChanged: (_) {
                          ...
                          try {
                            ...
                            if (raw.isEmpty) {
                              ...
                              final f = _fiatFmt.parse(raw).toDouble();
                              final int valueZats =
                                  ((f * _unitScale) / _fxRate!).round();
                              _zecCtl.text = formatAssetUnits(
                                valueZats,
                                _currentTokenPrecision,
                              );
                              _insufficient =
                                  valueZats > widget.availableZatoshis;
