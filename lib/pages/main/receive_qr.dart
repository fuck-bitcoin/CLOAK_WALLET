...
class ReceiveQrPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Determine coin-specific title
    final coinTicker = CloakWalletManager.isCloak(aa.coin) ? 'CLOAK' : 'ZEC';
...
class _ReceiveAddressPanelsState extends State<ReceiveAddressPanels> {
  ...
  String? _cloakReceiveAddress; // Generated once on page open
  ...
  void initState() {
    super.initState();
    // Generate a new CLOAK address exactly once when the Receive page opens
    if (CloakWalletManager.isCloak(aa.coin)) {
      _cloakReceiveAddress = CloakWalletManager.getAddress() ?? '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (context) {
      ...
      // CLOAK uses different address handling
      if (CloakWalletManager.isCloak(aa.coin)) {
        final cloakAddress = _cloakReceiveAddress ?? '';
        return Column(
          children: [
            _AddressPanel(
              icon: Icons.shield_outlined,
              title: 'CLOAK Shielded Address',
              subtitle: 'Your private receiving address',
              address: cloakAddress,
              expanded: true,
              onTap: () {},
              onInfo: () => _showInfo(
                context,
                'CLOAK Shielded Address',
                '- Use this address to receive CLOAK tokens privately.\n\n'
                '- All transactions to this address are encrypted and shielded using zero-knowledge proofs.\n\n'
                '- Your balance and transaction history remain completely private on the Telos blockchain.',
              ),
              onCopy: () => _copy(context, cloakAddress),
              onQr: () {
                if (cloakAddress.isNotEmpty) _showQr(context, 'CLOAK Shielded Address', cloakAddress);
              },
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            primaryButton(label: S.of(context).ok, onTap: () => Navigator.of(context).pop(true)),
          ],
        ),
        actions: const [],
      ),
        ...
        decoration: BoxDecoration(
          ...
          gradient: isShielded
              ? LinearGradient(
                  ...
                  colors: isCloak
                      ? const [cloakNavy, cloakGold]
                      : const [orangeDark, orangeBase],
                  stops: const [0.0, 1.0],
                )
              : null,
        ...
        child: Column(
          ...
          children: [
            Row(
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                // Coin glyph with shield overlay for shielded
                Container(
                  width: 40,
                  height: 40,
                  decoration: BoxDecoration(
