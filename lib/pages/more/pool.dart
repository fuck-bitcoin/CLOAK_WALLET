...
class _PoolTransferState extends State<PoolTransferPage>
  ...
  ok() async {
    ...
    load(() async {
      try {
        print('[POOL_TRANSFER] from=$from, to=$to, amount=$amount');
        print('[POOL_TRANSFER] Pool values: 1=transparent, 2=sapling, 4=orchard');
        ...
        // Get fee from the plan report
        final report = WarpApi.transactionReport(aa.coin, plan);
        final fee = report.fee;

        // Register pending migration so balance shows correctly during confirmation
