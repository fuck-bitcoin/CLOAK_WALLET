...
class SendingOverlayController {
  OverlayEntry? _entry;
  DateTime? _shownAt;
  final ValueNotifier<String> status = ValueNotifier<String>('Sending…');
  ...
  void show(BuildContext context) {
                    ...
                    child: ConstrainedBox(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                          const BeatPulse(color: Color(0xFFF4B728), size: 108, duration: Duration(milliseconds: 1400)),
                            const SizedBox(height: 8),
                            const SendingEllipses(duration: Duration(milliseconds: 2000)),
    ...
    scheduleMicrotask(() { try { _fade.value = 1.0; } catch (_) {} });
  }

  void setStatus(String text) {
    try { status.value = text; } catch (_) {}
  }
  ...
  Future<void> hide() async {
    try {
      ...
      if (shown != null) {
        ...
        final baseWait = minDurationMs - elapsed;
        // Start fade-out
        try { _fade.value = 0.0; } catch (_) {}
        final int fadeMs = 300;
        final int waitMs = baseWait > fadeMs ? baseWait : fadeMs;
...
class AmountPickerState extends State<AmountPicker> {
  ...
  Future<void> updateFxRate() async {
    // Use cached price immediately so USD field is enabled right away
    final cached = marketPrice.price;
    if (cached != null && fxRate == null) {
      fxRate = cached;
      setState(() {});
    }
    // Always fetch fresh price in background and update when ready
    final c = coins[aa.coin];
    final fresh = await getFxRate(c.currency, appSettings.currency);
    if (fresh != null) {
      fxRate = fresh;
      setState(() {});
    }
...
class MessageContentWidget extends StatelessWidget {
  ...
  MessageContentWidget(this.address, this.message, this.memo,
      {this.displayLabel, this.subtitle, this.inlineIcon, this.timestamp});
