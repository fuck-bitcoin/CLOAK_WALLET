...
/// Format a fiat value with dynamic precision for very small amounts.
/// For values >= 0.01, uses 2 decimal places (standard currency).
/// For smaller values, shows enough decimals for 2 significant figures
/// so that e.g. $0.000493 displays as "USD0.0005" instead of "USD0.00".
String formatFiatDynamic(double x, {String symbol = ''}) {
  final abs = x.abs();
  if (abs == 0) return decimalFormat(x, 2, symbol: symbol);
  if (abs >= 0.01) return decimalFormat(x, 2, symbol: symbol);
  // Find how many decimal places needed for 2 significant figures
  int digits = 2;
  ...
  digits++; // one extra for a second significant digit
  if (digits > 8) digits = 8;
  return decimalFormat(x, digits, symbol: symbol);
}
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              if (dismissable)
                primaryButton(label: label ?? s.ok, onTap: () => Navigator.of(dialogContext).pop(true)),
            ],
          ),
          actions: const [],
        ),
...
// Dynamic ZEC formatter that adjusts decimal places by magnitude
// - >= 1 ZEC: 3 decimals
// - 0.1–<1 ZEC: 4 decimals
// - 0.01–<0.1 ZEC: 5 decimals
// - 0.001–<0.01 ZEC: 6 decimals
// - 0.0001–<0.001 ZEC: 7 decimals
// - < 0.0001 ZEC: 8 decimals
int _dynamicZecDigitsForZats(int zats) {
  final int absZ = zats.abs();
  if (absZ >= ZECUNIT_INT) return 3;            // >= 1 ZEC
  if (absZ >= 10000000) return 4;               // >= 0.1 ZEC
  if (absZ >= 1000000) return 5;                // >= 0.01 ZEC
  if (absZ >= 100000) return 6;                 // >= 0.001 ZEC
  if (absZ >= 10000) return 7;                  // >= 0.0001 ZEC
  return 8;                                     // tiny amounts
}

String amountToStringDynamic(int amountZats) {
  // Fast path for zero
  if (amountZats == 0) return '0';
  final int dd = _dynamicZecDigitsForZats(amountZats);
  final String s = decimalFormat(amountZats / ZECUNIT, dd);
  // Trim forced trailing zeros and trailing decimal point for cleaner display
  return s
...
Future<double?> getFxRate(String coin, String fiat) async {
  // CLOAK: derive price from TLOS/USD × CLOAK/TLOS auction price
  if (coin == 'cloak') {
    return _getCloakFxRate(fiat);
  }

  final cgBase = 'api.coingecko.com';
  final cgUri = Uri.https(cgBase, '/api/v3/simple/price', {
    'ids': coin,
  ...
  try {
    ...
    if (rep.statusCode == 200) {
      ...
      return (p is double) ? p : (p as num).toDouble();
    }
  } catch (e) {
    logger.d('CoinGecko fx failed: $e');
  }
  // Fallback: CoinCap USD only (if fiat is USD)
  try {
    if (fiat.toUpperCase() == 'USD') {
      ...
      if (rep.statusCode == 200) {
        final json = convert.jsonDecode(rep.body) as Map<String, dynamic>;
        final priceStr = (json['data']?['priceUsd'] ?? '').toString();
        final v = double.tryParse(priceStr);
        if (v != null) return v;
      }
    }
  } catch (e) {
    logger.d('CoinCap fx failed: $e');
  }
  return null;
}
...
/// Fetch current CLOAK/TLOS price from the on-chain Dutch auction.
/// Price = total_TLOS_contributed_this_round / tokens_per_round
Future<double?> _getCloakTlosPrice() async {
  const endpoint = 'https://telos.eosusa.io';
  ...
  try {
    ...
    final startTime = (cfg['start_block_time'] as num).toInt();
    final roundDuration = (cfg['round_duration_sec'] as num).toInt();
    ...
    final auctResp = await http.post(
      Uri.parse('$endpoint/v1/chain/get_table_rows'),
      headers: {'Content-Type': 'application/json'},
