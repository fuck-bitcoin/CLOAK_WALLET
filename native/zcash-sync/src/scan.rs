...
async fn sync_async_inner<'a>(
    ...
    log::info!("=== SYNC START === coin={}, account={}, get_tx={}, target_height_offset={}, max_cost={}",
        coin, account, get_tx, target_height_offset, max_cost);
    
    let c = CoinConfig::get(coin);
    let ld_url = c.lwd_url.as_ref().unwrap().clone();
    log::info!("[SYNC] Lightwalletd URL: {}", ld_url);

    let network = *c.chain.network();
    log::info!("[SYNC] Network: {:?}", network);

    let mut client = connect_lightwalletd(&ld_url).await?;
    log::info!("[SYNC] Connected to lightwalletd");
    ...
    let (start_height, prev_hash, sapling_vks, orchard_vks) = {
        ...
        log::info!("[SYNC] DB state: height={}, sapling_vks_count={}, orchard_vks_count={}",
            height, sapling_vks.len(), orchard_vks.len());
        (height, hash, sapling_vks, orchard_vks)
    };
    
    let server_height = get_latest_height(&mut client).await?;
    log::info!("[SYNC] Server latest height: {}", server_height);
    
    let end_height = (server_height - target_height_offset).max(start_height);
    log::info!("[SYNC] Calculated end_height: {} (server {} - offset {} = {}, max with start {} = {})",
        end_height, server_height, target_height_offset, server_height - target_height_offset, start_height, end_height);
    
    if start_height >= end_height {
        log::info!("[SYNC] Already synced: start_height {} >= end_height {}", start_height, end_height);
        return Ok(());
    }
    
    log::info!("[SYNC] Will sync from height {} to {}", start_height, end_height);
    ...
    let mut total_sapling_notes = 0u64;
    let mut total_orchard_notes = 0u64;
    ...
    while let Some(blocks) = blocks_rx.recv().await {
        ...
        blocks_processed += blocks.0.len() as u64;
        
        // Log progress every 10000 blocks or near NU6/NU6_1 activation
        if blocks_processed % 10000 < blocks.0.len() as u64 || 
           (last_height >= 2720000 && last_height <= 2730000) ||
           (last_height >= 3140000 && last_height <= 3150000) {
            log::info!("[SYNC] Processing blocks {} to {} (total: {}, target: {})",
                first_block.height, last_height, blocks_processed, end_height);
        }
        ...
        let mut connection = c.connection();
        {
            ...
            // Sapling
            {
                ...
                let sapling_notes = synchronizer.process(&blocks.0)? as u64;
                if sapling_notes > 0 {
                    log::info!("[SYNC] *** SAPLING NOTE FOUND *** count={} at height={}", sapling_notes, last_height);
                    total_sapling_notes += sapling_notes;
                }
                progress.trial_decryptions += sapling_notes;
            }
            ...
            if c.chain.has_unified() {
                // Orchard
                {
                    ...
                    synchronizer.initialize(height)?;
                    let orchard_notes = synchronizer.process(&blocks.0)? as u64;
                    if orchard_notes > 0 {
                        log::info!("[SYNC] *** ORCHARD NOTE FOUND *** count={} at height={}", orchard_notes, last_height);
                        total_orchard_notes += orchard_notes;
                    }
                    progress.trial_decryptions += orchard_notes;
                }
            }
        ...
        // Log balance updates every 50000 blocks
        if blocks_processed % 50000 < blocks.0.len() as u64 {
            log::info!("[SYNC] Balances at height {}: T={}, S={}, O={}",
                last_height, progress.balances.transparent, progress.balances.sapling, progress.balances.orchard);
        }
    ...
    log::info!("[SYNC] === SHIELDED SYNC COMPLETE === blocks={}, sapling_notes={}, orchard_notes={}, final_height={}",
        blocks_processed, total_sapling_notes, total_orchard_notes, height);

    let download_result = downloader.await?;
    if let Err(ref e) = download_result {
        log::error!("[SYNC] Downloader error: {:?}", e);
    }
    download_result?;

    log::info!("[SYNC] Starting transparent sync to height {}", end_height);
    let mut client = connect_lightwalletd(&ld_url).await?;
    let trp_result = crate::taddr::transparent_sync(&network, c.connection(), &mut client, account, end_height).await;
    if let Err(ref e) = trp_result {
        log::error!("[SYNC] Transparent sync error: {:?}", e);
    }
    trp_result?;
    log::info!("[SYNC] Transparent sync complete");

    let cancel_token = SYNC_CANCEL.lock().as_ref().cloned();
    let cancelled = cancel_token.map(|c| c.is_cancelled()).unwrap_or_default();
    log::info!("[SYNC] Cancelled={}", cancelled);

    if !cancelled && get_tx {
        log::info!("[SYNC] Getting transaction details...");
        get_transaction_details(coin).await?;
        log::info!("[SYNC] Transaction details complete");
    }
    ...
    // Final balance check
    let final_balances = get_pool_balances_inner(&connection, height, account, false)?;
    log::info!("=== SYNC COMPLETE === final_height={}, T={}, S={}, O={}",
        height, final_balances.transparent, final_balances.sapling, final_balances.orchard);
...
pub fn get_pool_balances_inner(
    ...
    log::debug!("[BALANCE] get_pool_balances_inner: account={}, height={}, include_unconfirmed={}", 
        account, height, include_unconfirmed);
    ...
    // Count notes in database for debugging
    let sapling_note_count: u32 = connection.query_row(
        "SELECT COUNT(*) FROM received_notes WHERE account = ?1 AND orchard = 0",
        [account], |r| r.get(0)
    ).unwrap_or(0);
    let orchard_note_count: u32 = connection.query_row(
        "SELECT COUNT(*) FROM received_notes WHERE account = ?1 AND orchard = 1",
        [account], |r| r.get(0)
    ).unwrap_or(0);
    log::info!("[BALANCE] Note counts for account {}: sapling={}, orchard={}", 
        account, sapling_note_count, orchard_note_count);
    ...
    log::info!("[BALANCE] Final balances for account {}: T={}, S={}, O={}", 
        account, transparent, sapling, orchard);
