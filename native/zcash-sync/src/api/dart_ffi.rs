...
/// Non-blocking version of warp() that spawns a background thread and returns immediately.
/// Progress is reported via the port. Completion is signaled by a progress message with height=0xFFFFFFFF.
/// Result codes sent via completion: 0=success, 1=reorg, 2=busy, 255=error
#[no_mangle]
pub unsafe extern "C" fn warp_async(
    coin: u8,
    account: u32,
    get_tx: bool,
    anchor_offset: u32,
    ...
    // Capture POST_COBJ once at call time (on main thread) to avoid unsafe access from background
    let post_cobj = POST_COBJ;
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            let permit = SYNC_LOCK.acquire().await;
            if !permit.is_ok() {
                send_sync_completion_with(port, 2, post_cobj); // busy
                return;
            }
            *SYNC_CANCEL.lock() = Some(CancellationToken::new());
            log::info!("Sync started (async)");

            // Throttle progress updates to reduce FFI overhead
            ...
            let result = crate::api::sync::coin_sync(
                ...
                max_cost,
                move |progress| {
                    // Throttle: only send progress at most every 100ms
                    ...
                    *last = now;
                    drop(last); // Release lock before FFI call
            ...
            *SYNC_CANCEL.lock() = None;
            ...
            send_sync_completion_with(port, code, post_cobj);
        });
    });
}

/// Send a completion message via the port using a special progress message with height=0xFFFFFFFF
fn send_sync_completion_with(port: i64, code: u8, post_cobj: Option<ffi::DartPostCObjectFnType>) {
    use crate::db::data_generated::fb::PoolBalanceT;
    let progress = ProgressT {
        height: 0xFFFFFFFF, // Magic value to signal completion
        timestamp: code as u32, // Reuse timestamp field for result code
        trial_decryptions: 0,
        downloaded: 0,
        balances: Some(Box::new(PoolBalanceT::default())),
    };
    let v = fb_to_bytes!(progress);
    let mut progress = v.into_dart();
    if port != 0 {
        if let Some(p) = post_cobj {
            unsafe { p(port, &mut progress); }
        }
    }
}
...
/// Check if an account index has any on-chain activity (transparent UTXOs)
/// This is a lightweight check used for account discovery during seed restoration.
/// Returns 1 if activity detected, 0 if no activity, or error.
/// 
/// Note: The wallet uses BIP-44 path m/44'/coin'/0'/0/{aindex} where aindex is the
/// account index stored in the DB. The third component (account) is always 0'.
#[tokio::main]
#[no_mangle]
pub async unsafe extern "C" fn check_account_activity(
    coin: u8,
    seed: *mut c_char,
    account_index: u32,
) -> CResult<u8> {
    ...
    let res = async move {
        ...
        // Derive transparent address for this account index
        // Path format: m/44'/coin_type'/0'/0/{account_index}
        // This matches how the wallet creates transparent addresses in db.rs create_taddr()
        let bip44_path = format!("m/44'/{}'/0'/0/{}", network.coin_type(), account_index);
        let (_, taddr) = crate::taddr::derive_tkeys(network, &seed, &bip44_path)?;
        
        // Check if there are any UTXOs for this address
        let balance = crate::taddr::get_taddr_balance(&mut client, &taddr).await?;
        if balance > 0 {
            return Ok(1u8); // Activity detected
        }
        
        // Also check transaction count (may have spent UTXOs)
        let latest = crate::chain::get_latest_height(&mut client).await?;
        let range = crate::BlockRange {
            start: Some(crate::BlockId {
                height: 419200, // Sapling activation
                hash: vec![],
            }),
            end: Some(crate::BlockId {
                height: latest as u64,
                hash: vec![],
            }),
            ..Default::default()
        };
        let tx_count = crate::taddr::get_taddr_tx_count(&mut client, &taddr, &range).await?;
        if tx_count > 0 {
            return Ok(1u8); // Activity detected
        }
...
/// Scan for shielded account activity using ZIP-32 derivation with trial decryption.
/// This properly detects accounts by checking if viewing keys can decrypt notes.
/// 
/// Downloads recent blocks once and trial-decrypts with viewing keys derived at each index.
/// Scans both Sapling and Orchard pools, plus transparent addresses.
/// Returns a bitmask of which indexes (0-9) have activity detected.
/// Bit 0 = index 0, Bit 1 = index 1, etc.
/// 
/// If no activity found in quick scan, automatically does a deeper scan (more blocks).
/// 
/// Parameters:
/// - coin: coin type (0=ZEC, 1=YEC)  
/// - seed: seed phrase
/// - max_index: maximum index to scan (0-9, typically 10)
/// - num_blocks: number of recent blocks to scan (1000-5000 recommended)

/// DEBUG: Derive FVK from seed for testing
#[no_mangle]
pub unsafe extern "C" fn derive_fvk_for_index(
    coin: u8,
    seed: *mut c_char,
    index: u32,
) -> CResult<*mut c_char> {
    ...
    let res = || {
        ...
        let (phrase, password) = crate::key2::split_key(&seed);
        let mnemonic = Mnemonic::from_phrase(&phrase, Language::English)
            .map_err(|_| anyhow::anyhow!("Invalid seed phrase"))?;
        let bip39_seed = Bip39Seed::new(&mnemonic, &password);
        
        let master = ExtendedSpendingKey::master(bip39_seed.as_bytes());
        let path = [
...
#[tokio::main]
#[no_mangle]
pub async unsafe extern "C" fn scan_shielded_accounts(
    coin: u8,
    seed: *mut c_char,
    max_index: u32,
    num_blocks: u32,
    ...
    let res = async move {
        ...
        use crate::sapling::{SaplingDecrypter, SaplingViewKey};
        use crate::orchard::{OrchardDecrypter, OrchardViewKey};
        use crate::sync::TrialDecrypter;
        use orchard::keys::{SpendingKey as OrchardSpendingKey, FullViewingKey as OrchardFvk};
        use zcash_client_backend::encoding::encode_extended_full_viewing_key;
        use zcash_primitives::consensus::Parameters;
        
        eprintln!("[SCAN] ========== SCAN_SHIELDED_ACCOUNTS START ==========");
        eprintln!("[SCAN] coin={}, max_index={}, num_blocks={}", coin, max_index, num_blocks);
        
        let c = CoinConfig::get(coin);
        let network = c.chain.network();
        eprintln!("[SCAN] Network coin_type: {}", network.coin_type());
        
        let mut client = c.connect_lwd().await?;
        eprintln!("[SCAN] Connected to lightwalletd");
        
        // Parse seed phrase
        let (phrase, password) = crate::key2::split_key(&seed);
        eprintln!("[SCAN] Seed phrase word count: {}, has password: {}", 
            phrase.split_whitespace().count(), !password.is_empty());
        ...
        // Get latest height
        let latest_height = crate::chain::get_latest_height(&mut client).await?;
        eprintln!("[SCAN] Latest blockchain height: {}", latest_height);
        
        let max_index = max_index.min(9); // Cap at index 9 (10 accounts max)
        
        // Derive Sapling viewing keys for each index
        let master = ExtendedSpendingKey::master(bip39_seed.as_bytes());
        let mut sapling_vks = Vec::new();
        for index in 0..=max_index {
            ...
            // Log the encoded FVK for index 0 so we can compare with database
            if index == 0 {
                let encoded_fvk = encode_extended_full_viewing_key(
                    network.hrp_sapling_extended_full_viewing_key(),
                    &fvk,
                );
                eprintln!("[SCAN] Index 0 Sapling FVK (first 80 chars): {}...", 
                    &encoded_fvk[..80.min(encoded_fvk.len())]);
            }
            
            sapling_vks.push(SaplingViewKey { account: index, fvk, ivk });
        }
        
        // Derive Orchard viewing keys for each index
        let mut orchard_vks = Vec::new();
        for index in 0..=max_index {
            if let Ok(sk) = OrchardSpendingKey::from_zip32_seed(bip39_seed.as_bytes(), network.coin_type(), index) {
                let fvk = OrchardFvk::from(&sk);
                orchard_vks.push(OrchardViewKey { account: index, fvk });
            }
        }
        
        eprintln!("[SCAN] Created {} Sapling and {} Orchard viewing keys for indexes 0-{}", 
            sapling_vks.len(), orchard_vks.len(), max_index);
        
        // Helper function to scan blocks for shielded activity
        let scan_blocks = |blocks: &[crate::CompactBlock], sapling_vks: &[SaplingViewKey], orchard_vks: &[OrchardViewKey], network: &Network| -> u32 {
            let mut found: u32 = 0;
            let sapling_decrypter = SaplingDecrypter::new(*network);
            let orchard_decrypter = OrchardDecrypter::new(*network);
            
            let mut total_sapling_outputs = 0u64;
            let mut total_orchard_actions = 0u64;
            let mut blocks_with_sapling = 0u32;
            let mut blocks_with_orchard = 0u32;
            
            for block in blocks.iter() {
                let mut block_sapling_outputs = 0;
                let mut block_orchard_actions = 0;
                
                // Count outputs in this block
                for vtx in block.vtx.iter() {
                    block_sapling_outputs += vtx.outputs.len();
                    block_orchard_actions += vtx.actions.len();
                }
                
                if block_sapling_outputs > 0 {
                    blocks_with_sapling += 1;
                    total_sapling_outputs += block_sapling_outputs as u64;
                }
                if block_orchard_actions > 0 {
                    blocks_with_orchard += 1;
                    total_orchard_actions += block_orchard_actions as u64;
                }
                
                // Sapling trial decryption
                let sapling_decrypted = sapling_decrypter.decrypt_notes(block, sapling_vks);
                for tx in sapling_decrypted.txs.iter() {
                    if !tx.notes.is_empty() {
                        eprintln!("[SCAN] *** Found {} SAPLING note(s) at index {} in block {} ***", 
                            tx.notes.len(), tx.account, block.height);
                        found |= 1 << tx.account;
                    }
                }
                
                // Orchard trial decryption
                let orchard_decrypted = orchard_decrypter.decrypt_notes(block, orchard_vks);
                for tx in orchard_decrypted.txs.iter() {
                    if !tx.notes.is_empty() {
                        eprintln!("[SCAN] *** Found {} ORCHARD note(s) at index {} in block {} ***", 
                            tx.notes.len(), tx.account, block.height);
                        found |= 1 << tx.account;
                    }
                }
            ...
            eprintln!("[SCAN] Block scan stats: {} blocks with {} sapling outputs, {} blocks with {} orchard actions",
                blocks_with_sapling, total_sapling_outputs, blocks_with_orchard, total_orchard_actions);
            
            found
        };
        
        // PHASE 1: Quick scan with requested block range
        let num_blocks_quick = num_blocks.min(50000).max(100);
        let start_height_quick = latest_height.saturating_sub(num_blocks_quick);
        
        eprintln!("[SCAN] Phase 1: Quick scan from height {} to {} ({} blocks)", 
            start_height_quick, latest_height, num_blocks_quick);
        
        // Download blocks
        let range = crate::BlockRange {
            start: Some(crate::BlockId { height: (start_height_quick + 1) as u64, hash: vec![] }),
            end: Some(crate::BlockId { height: latest_height as u64, hash: vec![] }),
            spam_filter_threshold: 0,
        };
        eprintln!("[SCAN] Requesting blocks from {} to {}", start_height_quick + 1, latest_height);
        
        let mut stream = client.get_block_range(tonic::Request::new(range)).await?.into_inner();
        let mut blocks = Vec::new();
        while let Some(block) = stream.message().await? {
            blocks.push(block);
        }
        eprintln!("[SCAN] Downloaded {} blocks for quick scan", blocks.len());
        
        if blocks.is_empty() {
            eprintln!("[SCAN] WARNING: No blocks downloaded! Check lightwalletd connection.");
        } else {
            eprintln!("[SCAN] First block height: {}, last block height: {}", 
                blocks.first().map(|b| b.height).unwrap_or(0),
                blocks.last().map(|b| b.height).unwrap_or(0));
        }
        
        let mut found_indexes = scan_blocks(&blocks, &sapling_vks, &orchard_vks, network);
        eprintln!("[SCAN] Phase 1 result: found_indexes bitmask = 0b{:b}", found_indexes);
        
        // PHASE 2: Check transparent activity (balance OR transaction history)
        eprintln!("[SCAN] Phase 2: Checking transparent activity");
        let transparent_range = crate::BlockRange {
            start: Some(crate::BlockId { height: 419200, hash: vec![] }), // Sapling activation
            end: Some(crate::BlockId { height: latest_height as u64, hash: vec![] }),
            ..Default::default()
        };
        ...
        for index in 0..=max_index {
            ...
            if let Ok((_, taddr)) = crate::taddr::derive_tkeys(network, &seed, &bip44_path) {
                if index == 0 {
                    eprintln!("[SCAN] Index 0 transparent address: {}", taddr);
                }
                let balance = crate::taddr::get_taddr_balance(&mut client, &taddr).await.unwrap_or(0);
                if balance > 0 {
                    eprintln!("[SCAN] Found transparent balance {} zatoshi at index {}", balance, index);
                    found_indexes |= 1 << index;
                } else {
                    let tx_count = crate::taddr::get_taddr_tx_count(&mut client, &taddr, &transparent_range).await.unwrap_or(0);
                    if tx_count > 0 {
                        eprintln!("[SCAN] Found transparent tx history ({} txs) at index {}", tx_count, index);
                        found_indexes |= 1 << index;
                    }
                }
            }
        }
        eprintln!("[SCAN] Phase 2 result: found_indexes bitmask = 0b{:b}", found_indexes);
        
        // PHASE 3: If nothing found, do a deeper scan (10x more blocks)
        if found_indexes == 0 {
            let deep_scan_blocks = (num_blocks_quick * 10).min(50000); // 10x more blocks, max 50k
            let start_height_deep = latest_height.saturating_sub(deep_scan_blocks);
            
            // Only scan additional blocks we haven't scanned yet
            if start_height_deep < start_height_quick {
                eprintln!("[SCAN] Phase 3: Deep scan from height {} to {} ({} additional blocks)", 
                    start_height_deep, start_height_quick, start_height_quick - start_height_deep);
                
                let range = crate::BlockRange {
                    start: Some(crate::BlockId { height: (start_height_deep + 1) as u64, hash: vec![] }),
                    end: Some(crate::BlockId { height: start_height_quick as u64, hash: vec![] }),
                    spam_filter_threshold: 0,
                };
                let mut stream = client.get_block_range(tonic::Request::new(range)).await?.into_inner();
                let mut deep_blocks = Vec::new();
                while let Some(block) = stream.message().await? {
                    deep_blocks.push(block);
                }
                eprintln!("[SCAN] Downloaded {} blocks for deep scan", deep_blocks.len());
                
                found_indexes = scan_blocks(&deep_blocks, &sapling_vks, &orchard_vks, network);
                
                if found_indexes != 0 {
                    eprintln!("[SCAN] Deep scan found activity! Bitmask: 0b{:b}", found_indexes);
                } else {
                    eprintln!("[SCAN] Deep scan found no activity - will default to index 0");
                }
            }
        }
        
        eprintln!("[SCAN] ========== SCAN COMPLETE ==========");
        eprintln!("[SCAN] Final result: found indexes bitmask = 0b{:b}", found_indexes);
        if found_indexes == 0 {
            eprintln!("[SCAN] No activity found - caller should default to index 0");
        } else {
            for i in 0..=max_index {
                if (found_indexes & (1 << i)) != 0 {
                    eprintln!("[SCAN] Found activity at index {}", i);
                }
            }
        }
