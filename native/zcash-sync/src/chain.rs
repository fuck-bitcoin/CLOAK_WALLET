...
pub async fn download_chain(
    ...
    log::info!("[DOWNLOAD] Starting download from {} to {}", start_height + 1, end_height);
    ...
    log::info!("[DOWNLOAD] Outputs per chunk = {}, max_cost = {}", outputs_per_chunk, max_cost);
    ...
    let mut last_logged_height = start_height;
    
    loop {
        tokio::select! {
            _ = cancel_token.cancelled() => {
                log::info!("[DOWNLOAD] Cancelled at {} blocks downloaded", blocks_downloaded);
                return Ok(());
            },
            message = block_stream.message() => {
                match message {
                    Err(e) => {
                        log::error!("[DOWNLOAD] Stream error after {} blocks: {:?}", blocks_downloaded, e);
                        return Err(e.into());
                    },
                    Ok(None) => {
                        log::info!("[DOWNLOAD] Stream ended after {} blocks", blocks_downloaded);
                        break;
                    },
                    Ok(Some(mut block)) => {
                    blocks_downloaded += 1;
                    ...
                    // Log progress every 10000 blocks or near NU6/NU6_1
                    if current_height - last_logged_height >= 10000 ||
                       (current_height >= 2720000 && current_height <= 2730000 && current_height != last_logged_height) ||
                       (current_height >= 3140000 && current_height <= 3150000 && current_height != last_logged_height) {
                        log::info!("[DOWNLOAD] Progress: height={}, blocks={}, target={}", 
                            current_height, blocks_downloaded, end_height);
                        last_logged_height = current_height;
                    }
    ...
    log::info!("[DOWNLOAD] Complete: {} blocks downloaded", blocks_downloaded);
