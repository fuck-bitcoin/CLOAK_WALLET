...
pub async fn transparent_sync(
    ...
    log::info!("[TRP_SYNC] Starting transparent sync: account={}, end_height={}", account, end_height);
        ...
        .optional()?
    {
        Some(r) => r,
        None => {
            log::info!("[TRP_SYNC] No t-addr found for account {}", account);
            return Ok(false); // no t-addr
        }
    };

    log::info!("[TRP_SYNC] T-addr: {}, start_height: {}", account_address, start_height);
    
    if start_height >= end_height {
        log::info!("[TRP_SYNC] Already synced: start {} >= end {}", start_height, end_height);
    ...
    db_tx.commit()?;
    log::info!("[TRP_SYNC] Transparent sync complete for account {}, synced to height {}", account, end_height);
    Ok(true)
}
