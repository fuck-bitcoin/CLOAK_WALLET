                ...
                Name(10639630974360485888) => Ok(ResolvedZAction{
                    ...
                    data: {
                        ...
                        if n.quantity().eq(&Asset::from_string(&"0").unwrap())
                        {
                            ...
                            // in case of auth note we re-create the Rseed based on a seed phrase placed into the memo.
                            // If the MintDesc memo is non-empty, use it directly as the vault seed (deterministic vaults).
                            // This ensures the on-chain commitment hash matches what createDeterministicVault produces.
                            // Otherwise, generate a random 24-word BIP-39 mnemonic (legacy behavior).
                            let memo = if desc.memo.trim().is_empty() {
...
pub fn create_auth_token(wallet: &Wallet, seed: String, contract: Name, recipient: Address) -> Result<HashMap<String, Vec<String>>, TransactionError>
{
    ...
    // add commitment hash under special key "__commitment__"
    // this is the 64-char hex string needed for AUTH memo format
    let commitment_hex = hex::encode(n.commitment().to_bytes());
    unpublished_notes
        .entry("__commitment__".to_string())
        .or_default()
        .push(commitment_hex);
