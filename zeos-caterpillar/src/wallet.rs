use chrono::{DateTime, Local};
use std::collections::{HashMap, HashSet};
use std::io::{self, Read, Write};
use std::time::{SystemTime, UNIX_EPOCH, Duration};
use byteorder::{ReadBytesExt, WriteBytesExt, LittleEndian};
use lazy_static::lazy_static;
use serde::{Serialize, Deserialize};
use serde_json::Value;
use crate::contract::{PlsSpendSequence, PlsAuthenticate};
use crate::eosio::ExtendedAsset;
use crate::{
    address::Address,
    blake2s7r::Params as Blake2s7rParams,
    constants::{MERKLE_TREE_DEPTH, MEMO_CHANGE_NOTE},
    contract::ScalarBytes,
    eosio::{Asset, Authorization, Name, Symbol},
    keys::{IncomingViewingKey, SpendingKey, FullViewingKey, PreparedIncomingViewingKey},
    note::{Note, NoteEx},
    note_encryption::{try_note_decryption, try_output_recovery_with_ovk, TransmittedNoteCiphertext},
    log
};
use crate::engine::{scalar_one, scalar_to_canonical_bytes};
use hmac::{Hmac, Mac};
use sha2::Sha256;

type HmacSha256 = Hmac<Sha256>;

// empty merkle tree roots
lazy_static! {
    static ref EMPTY_ROOTS: Vec<ScalarBytes> = {
        let mut v = vec![ScalarBytes(scalar_to_canonical_bytes(&scalar_one()))];
        for d in 0..MERKLE_TREE_DEPTH
        {
            let next = Blake2s7rParams::new()
                .hash_length(32)
                .personal(crate::constants::MERKLE_TREE_PERSONALIZATION)
                .to_state()
                .update(&v[d].0)
                .update(&v[d].0)
                .finalize()
                .as_bytes()
                .try_into()
                .expect("output length is correct");
            v.push(ScalarBytes(next));
        }
        v
    };
}

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct Wallet
{
    // keys & addresses
    seed: Vec<u8>,
    ivk: IncomingViewingKey, // READ-ONLY WALLET: only valid if seed == ""
    #[serde(default)]
    fvk_bytes: Option<Vec<u8>>, // Full Viewing Key bytes (128 bytes: dk+ak+nk+ovk) for FVK wallets
    diversifiers: Vec<u64>,

    // wallet metadata
    chain_id: [u8; 32],
    protocol_contract: Name,
    vault_contract: Name,
    alias_authority: Authorization,
    block_num: u32,
    leaf_count: u64,
    auth_count: u64,

    // the different note pools
    unspent_notes: Vec<NoteEx>,
    spent_notes: Vec<NoteEx>,
    outgoing_notes: Vec<NoteEx>,

    // merkle tree leaves
    merkle_tree: HashMap<u64, ScalarBytes>,

    // storage of all unpublished notes
    unpublished_notes: HashMap<u64, HashMap<String, Vec<String>>>
}

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct HistoryTransaction
{
    pub tx_type: String,
    pub date_time: String,
    pub tx_fee: String,
    pub timestamp_ms: u64,
    pub sort_order: u32,  // lower = earlier in display (0=fee, 1=main action)
    pub account_asset_memo: Vec<(String, String, String)>  // asset, receiver, memo
}

impl Wallet
{
    pub fn create(
        seed: &[u8],
        is_ivk: bool,
        chain_id: [u8; 32],
        protocol_contract: Name,
        vault_contract: Name,
        alias_authority: Authorization,
        fvk_bytes: Option<Vec<u8>>,
    ) -> Option<Self>
    {
        if is_ivk { if seed.len() != 64 { log("ivk length must equal 64 bytes"); return None; } }
        else      { if seed.len() < 32  { log("seed length must equal at least 32 bytes"); return None; } }

        let ivk = if is_ivk {
            let ivk = IncomingViewingKey::from_bytes(&seed.try_into().unwrap());
            if ivk.is_none().into() { log("ivk invalid"); return None; }
            ivk.unwrap()
        } else {
            FullViewingKey::from_spending_key(&SpendingKey::from_seed(seed)).ivk()
        };

        let diversifiers = if is_ivk {
            vec![]
        } else {
            vec![u64::try_from(FullViewingKey::from_spending_key(&SpendingKey::from_seed(seed)).default_address().0).unwrap()]
        };

        Some(Wallet{
            seed: if !is_ivk { seed.to_vec() } else { vec![] },
            ivk,
            fvk_bytes,
            diversifiers,
            chain_id,
            protocol_contract,
            vault_contract,
            alias_authority,
            block_num: 0,
            leaf_count: 0,
            auth_count: 0,
            unspent_notes: vec![],
            spent_notes: vec![],
            outgoing_notes: vec![],
            merkle_tree: HashMap::new(),
            unpublished_notes: HashMap::new()
        })
    }

    pub fn write<W: Write>(&self, mut writer: W) -> io::Result<()>
    {
        writer.write_u32::<LittleEndian>(self.seed.len() as u32)?;
        writer.write_all(self.seed.as_ref())?;
        writer.write_all(self.ivk.to_bytes().as_ref())?;
        writer.write_u32::<LittleEndian>(self.diversifiers.len() as u32)?;
        for d in self.diversifiers.iter()
        {
            writer.write_u64::<LittleEndian>(*d)?;
        }
        writer.write_all(&self.chain_id)?;
        writer.write_u64::<LittleEndian>(self.protocol_contract.raw())?;
        writer.write_u64::<LittleEndian>(self.vault_contract.raw())?;
        writer.write_u64::<LittleEndian>(self.alias_authority.actor.raw())?;
        writer.write_u64::<LittleEndian>(self.alias_authority.permission.raw())?;
        writer.write_u32::<LittleEndian>(self.block_num)?;
        writer.write_u64::<LittleEndian>(self.leaf_count)?;
        writer.write_u64::<LittleEndian>(self.auth_count)?;
        writer.write_u32::<LittleEndian>(self.unspent_notes.len() as u32)?;
        for n in self.unspent_notes.iter()
        {
            n.write(&mut writer)?;
        }
        writer.write_u32::<LittleEndian>(self.spent_notes.len() as u32)?;
        for n in self.spent_notes.iter()
        {
            n.write(&mut writer)?;
        }
        writer.write_u32::<LittleEndian>(self.outgoing_notes.len() as u32)?;
        for n in self.outgoing_notes.iter()
        {
            n.write(&mut writer)?;
        }
        for li in 0..self.leaf_count
        {
            // calculate array index of this leaf in current tree
            let idx = MT_ARR_LEAF_ROW_OFFSET!(MERKLE_TREE_DEPTH) + li % MT_NUM_LEAVES!(MERKLE_TREE_DEPTH);
            // calculate tree offset to translate local array indices of current tree to absolute array indices of global array
            let tos = li / MT_NUM_LEAVES!(MERKLE_TREE_DEPTH) * MT_ARR_FULL_TREE_OFFSET!(MERKLE_TREE_DEPTH);
            let idx = tos + idx;
            assert!(self.merkle_tree.contains_key(&idx));
            writer.write_all(self.merkle_tree.get(&idx).unwrap().0.as_ref())?;
        }
        let unpublished_notes_str = serde_json::to_string(&self.unpublished_notes).unwrap();
        writer.write_u32::<LittleEndian>(unpublished_notes_str.as_bytes().len() as u32)?;
        writer.write_all(unpublished_notes_str.as_bytes())?;

        // Write FVK bytes (128 bytes) if present, 0 length if not
        match &self.fvk_bytes {
            Some(bytes) => {
                writer.write_u32::<LittleEndian>(bytes.len() as u32)?;
                writer.write_all(bytes)?;
            }
            None => {
                writer.write_u32::<LittleEndian>(0)?;
            }
        }

        Ok(())
    }

    pub fn read<R: Read>(mut reader: R) -> io::Result<Self>
    {
        let seed_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut seed = vec![];
        seed.resize(seed_len, 0);
        reader.read_exact(&mut seed)?;

        let mut ivk = [0; 64];
        reader.read_exact(&mut ivk)?;
        let ivk = IncomingViewingKey::from_bytes(&ivk);
        assert!(bool::from(ivk.is_some()));
        let ivk = ivk.unwrap();

        let diversifiers_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut diversifiers = vec![];
        for _ in 0..diversifiers_len
        {
            let d = reader.read_u64::<LittleEndian>()?;
            diversifiers.push(d);
        }

        let mut chain_id = [0; 32];
        reader.read_exact(&mut chain_id)?;
        let protocol_contract = Name(reader.read_u64::<LittleEndian>()?);
        let vault_contract = Name(reader.read_u64::<LittleEndian>()?);
        let alias_authority = Authorization{
            actor: Name(reader.read_u64::<LittleEndian>()?),
            permission: Name(reader.read_u64::<LittleEndian>()?)
        };
        let block_num = reader.read_u32::<LittleEndian>()?;
        let leaf_count = reader.read_u64::<LittleEndian>()?;
        let auth_count = reader.read_u64::<LittleEndian>()?;

        let unspent_notes_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut unspent_notes = vec![];
        for _ in 0..unspent_notes_len
        {
            let n = NoteEx::read(&mut reader)?;
            unspent_notes.push(n);
        }

        let spent_notes_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut spent_notes = vec![];
        for _ in 0..spent_notes_len
        {
            let n = NoteEx::read(&mut reader)?;
            spent_notes.push(n);
        }

        let outgoing_notes_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut outgoing_notes = vec![];
        for _ in 0..outgoing_notes_len
        {
            let n = NoteEx::read(&mut reader)?;
            outgoing_notes.push(n);
        }

        let merkle_tree = HashMap::new();

        let mut wallet = Wallet {
            seed,
            ivk,
            fvk_bytes: None, // will be set after reading from binary format
            diversifiers,
            chain_id,
            protocol_contract,
            vault_contract,
            alias_authority,
            block_num,
            leaf_count: 0,
            auth_count,
            unspent_notes,
            spent_notes,
            outgoing_notes,
            merkle_tree,
            unpublished_notes: HashMap::new()
        };

        for _ in 0..leaf_count
        {
            let mut leaf = [0; 32];
            reader.read_exact(&mut leaf)?;
            wallet.insert_into_merkle_tree(&ScalarBytes(leaf));
        }

        let unpublished_notes_bytes_len = reader.read_u32::<LittleEndian>()? as usize;
        let mut unpublished_notes_bytes = vec![];
        unpublished_notes_bytes.resize(unpublished_notes_bytes_len, 0);
        reader.read_exact(&mut unpublished_notes_bytes)?;
        wallet.unpublished_notes = serde_json::from_str(&String::from_utf8(unpublished_notes_bytes).unwrap()).unwrap();

        // Read FVK bytes if present (backward compatible: EOF means no FVK)
        wallet.fvk_bytes = match reader.read_u32::<LittleEndian>() {
            Ok(len) if len > 0 => {
                let mut bytes = vec![0u8; len as usize];
                reader.read_exact(&mut bytes)?;
                Some(bytes)
            }
            Ok(_) => None,
            Err(ref e) if e.kind() == io::ErrorKind::UnexpectedEof => None,
            Err(e) => return Err(e),
        };

        Ok(wallet)
    }

    pub fn to_json(&self, pretty: bool) -> String
    {
        if pretty { serde_json::to_string_pretty(self).unwrap() }
        else { serde_json::to_string(self).unwrap() }
    }

    pub fn from_json(json: &String) -> Result<Self, serde_json::error::Error>
    {
        serde_json::from_str(&json)
    }

    pub fn size(&self) -> usize
    {
        4 +                                 // seed.len()
        self.seed.len() +
        64 +                                // Incoming Viewing Key
        4 +                                 // diversifiers.len()
        self.diversifiers.len() * 8 +
        32 +                                // chain ID
        8 +                                 // protocol contract
        8 +                                 // vault contract
        16 +                                // alias authority
        4 +                                 // latest block number
        8 +                                 // leaf count
        8 +                                 // auth count
        4 +                                 // unspent_notes.len()
        self.unspent_notes.len() * 655 +
        4 +                                 // spent_notes.len()
        self.spent_notes.len() * 655 +
        4 +                                 // outgoing_notes.len()
        self.outgoing_notes.len() * 655 +
        self.leaf_count as usize * 32 +     // merkle tree leaves only
        4 +                                 // unpublished_notes.len()
        serde_json::to_string(&self.unpublished_notes).unwrap().as_bytes().len() +
        4 +                                 // fvk_bytes.len()
        self.fvk_bytes.as_ref().map_or(0, |b| b.len())

        // less verbose but much heavier alternative:
        //let mut v = vec![];
        //assert!(self.write(&mut v).is_ok());
        //v.len()
    }

    pub fn seed(&self) -> Vec<u8>
    {
        self.seed.to_vec()
    }

    pub fn chain_id(&self) -> [u8; 32]
    {
        self.chain_id
    }

    pub fn protocol_contract(&self) -> &Name
    {
        &self.protocol_contract
    }

    pub fn vault_contract(&self) -> &Name
    {
        &self.vault_contract
    }

    pub fn alias_authority(&self) -> &Authorization
    {
        &self.alias_authority
    }

    pub fn block_num(&self) -> u32
    {
        self.block_num
    }

    pub fn leaf_count(&self) -> u64
    {
        self.leaf_count
    }

    pub fn auth_count(&self) -> u64
    {
        self.auth_count
    }

    pub fn set_auth_count(&mut self, count: u64)
    {
        self.auth_count = count;
    }

    /// Returns the maximum block_ts across all note pools (unspent, spent, outgoing).
    /// Used by the eager update to ensure new notes sort at the top of TX history
    /// even when the local system clock is behind chain UTC time.
    pub fn max_block_ts(&self) -> u64
    {
        self.unspent_notes.iter().map(|n| n.block_ts())
            .chain(self.spent_notes.iter().map(|n| n.block_ts()))
            .chain(self.outgoing_notes.iter().map(|n| n.block_ts()))
            .max()
            .unwrap_or(0)
    }

    /// Mark notes as spent by checking their nullifiers against on-chain nullifiers.
    /// Takes a slice of 32-byte nullifier values. Returns the number of notes marked as spent.
    pub fn mark_notes_spent(&mut self, nullifiers: &[ScalarBytes]) -> usize
    {
        if nullifiers.is_empty() || self.unspent_notes.is_empty() {
            return 0;
        }

        // Get FVK: from seed (full wallet), from stored fvk_bytes (FVK wallet), or bail (IVK-only)
        let fvk = if !self.seed.is_empty() {
            let sk = SpendingKey::from_seed(&self.seed);
            FullViewingKey::from_spending_key(&sk)
        } else if let Some(reconstructed) = self.reconstruct_fvk() {
            reconstructed
        } else {
            // IVK-only wallet: cannot compute nullifiers without nk
            return 0;
        };
        let mut spent_count = 0;

        // Convert on-chain nullifiers to Scalars for comparison
        let nf_scalars: Vec<crate::engine::Scalar> = nullifiers.iter()
            .filter_map(|nf| {
                let s = crate::engine::Scalar::try_from(nf.clone());
                s.ok()
            })
            .collect();

        // Check each unspent note against all on-chain nullifiers
        // Iterate in reverse so we can remove by index safely
        let mut i = self.unspent_notes.len();
        while i > 0 {
            i -= 1;
            // Auth tokens use Mint proofs (not Spend), so no nullifiers exist on-chain.
            // Skip them — burn detection is handled by Dart via Telos API classification.
            if self.unspent_notes[i].note().is_auth_token() { continue; }
            let note_nf = self.unspent_notes[i].note().nullifier(&fvk.nk, self.unspent_notes[i].position()).extract().0;
            if nf_scalars.iter().any(|nf| note_nf.eq(nf)) {
                let note = self.unspent_notes.remove(i);
                self.spent_notes.push(note);
                spent_count += 1;
            }
        }

        spent_count
    }

    /// Move a single note by index from unspent to spent.
    /// Used by wallet_transact_packed to eagerly reflect spent inputs after signing.
    pub fn mark_note_spent(&mut self, index: usize) {
        if index < self.unspent_notes.len() {
            let note = self.unspent_notes.remove(index);
            self.spent_notes.push(note);
        }
    }

    /// Eagerly handle a burned auth token (burn=1 authenticate).
    /// Moves the auth token from unspent → spent and creates a synthetic outgoing entry
    /// so transaction_history() can populate burn_timestamps for "Burn Vault" labeling.
    /// Called from the eager wallet update in lib.rs after zsign_transaction.
    pub fn burn_auth_token_eagerly(&mut self, cm_hex: &str, block_ts: u64) -> bool {
        let idx = self.unspent_notes.iter().position(|n| {
            n.note().is_auth_token() &&
            hex::encode(n.note().commitment().to_bytes()) == cm_hex
        });

        if let Some(i) = idx {
            let burned = self.unspent_notes.remove(i);
            log("[EAGER] Auth token burned, moving to spent+outgoing");

            // Create synthetic outgoing entry (mirrors digest_block wallet.rs:1188-1195)
            let burn_ex = NoteEx::from_parts(0, block_ts, block_ts, 0, burned.note().clone());
            self.outgoing_notes.push(burn_ex);

            // Move original to spent
            self.spent_notes.push(burned);

            true
        } else {
            log("[EAGER] Auth token not found in unspent (may already be spent)");
            false
        }
    }

    /// Reset all chain state to empty, preserving unpublished_notes.
    /// Called when user wants to resync from scratch (e.g., corrupted state).
    /// Caller MUST call wallet.write() after this to persist changes.
    pub fn reset_chain_state(&mut self) {
        log("[RESET] Clearing all chain state (block_num, leaves, notes, merkle_tree)");

        // Reset chain sync position
        self.block_num = 0;
        self.leaf_count = 0;
        self.auth_count = 0;

        // Clear note pools
        self.unspent_notes.clear();
        self.spent_notes.clear();
        self.outgoing_notes.clear();

        // Clear merkle tree
        self.merkle_tree.clear();

        // Preserve: seed, ivk, diversifiers, chain_id, contracts, unpublished_notes
        log("[RESET] Chain state cleared. Wallet ready for full resync.");
    }

    /// Clear all unpublished notes (auth tokens stored locally for vaults).
    /// Called before re-importing vaults from DB to prevent duplicate accumulation.
    /// Caller MUST call wallet.write() after this to persist changes.
    pub fn clear_unpublished_notes(&mut self) {
        self.unpublished_notes.clear();
    }

    pub fn is_ivk(&self) -> bool
    {
        self.seed.len() == 0
    }

    /// Returns true if this is a Full Viewing Key wallet (has stored FVK bytes but no seed).
    pub fn has_fvk(&self) -> bool
    {
        self.seed.is_empty() && self.fvk_bytes.is_some()
    }

    /// Reconstruct FullViewingKey from stored fvk_bytes. Returns None for IVK-only or seed wallets.
    fn reconstruct_fvk(&self) -> Option<FullViewingKey>
    {
        self.fvk_bytes.as_ref().and_then(|bytes| {
            FullViewingKey::read(&bytes[..]).ok()
        })
    }

    /// Get the Incoming Viewing Key as bech32m encoded string (ivk1...)
    pub fn ivk_bech32m(&self) -> Result<String, bech32::Error>
    {
        self.ivk.to_bech32m()
    }

    /// Get the Full Viewing Key as bech32m encoded string (fvk1...)
    /// Returns None if this is an IVK-only wallet
    pub fn fvk_bech32m(&self) -> Option<Result<String, bech32::Error>>
    {
        if !self.seed.is_empty() {
            let fvk = FullViewingKey::from_spending_key(&SpendingKey::from_seed(&self.seed));
            Some(fvk.to_bech32m())
        } else if let Some(fvk) = self.reconstruct_fvk() {
            Some(fvk.to_bech32m())
        } else {
            None
        }
    }

    /// Get the Outgoing Viewing Key as bech32m encoded string (ovk1...)
    /// Returns None if this is an IVK-only wallet
    pub fn ovk_bech32m(&self) -> Option<Result<String, bech32::Error>>
    {
        if !self.seed.is_empty() {
            let fvk = FullViewingKey::from_spending_key(&SpendingKey::from_seed(&self.seed));
            Some(fvk.ovk().to_bech32m())
        } else if let Some(fvk) = self.reconstruct_fvk() {
            Some(fvk.ovk().to_bech32m())
        } else {
            None
        }
    }

    pub fn balances(&self) -> Vec<ExtendedAsset>
    {
        let mut map = HashMap::new();
        for note in &self.unspent_notes
        {
            if !note.note().quantity().is_nft()
            {
                let k = ((note.note().symbol().raw() as u128) << 64) | (note.note().contract().raw() as u128);
                if !map.contains_key(&k)
                {
                    map.insert(k, note.note().amount());
                }
                else
                {
                    *map.get_mut(&k).unwrap() += note.note().amount();
                }
            }
        }
        let mut v = vec![];
        for k in map.keys()
        {
            v.push(ExtendedAsset::new(Asset::new(
                *map.get(k).unwrap() as i64,
                Symbol((*k >> 64) as u64)).unwrap(),
                Name(*k as u64)
            ))
        }
        v
    }

    pub fn unspent_notes(&self) -> &Vec<NoteEx>
    {
        &self.unspent_notes
    }

    pub fn fungible_tokens(&self, symbol: &Symbol, contract: &Name) -> Vec<NoteEx>
    {
        if symbol.raw() == 0 && contract.raw() == 0
        {
            // select all fungible tokens from all contracts
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().symbol().raw() != 0).collect();
        }
        if symbol.raw() == 0 && contract.raw() != 0
        {
            // select all fungible tokens from this particular contract
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().contract().eq(&contract) && n.note().symbol().raw() != 0).collect();
        }
        if symbol.raw() != 0 && contract.raw() == 0
        {
            // select this particular fungible token from all contracts
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().symbol().eq(&symbol)).collect();
        }
        if symbol.raw() != 0 && contract.raw() != 0
        {
            // select this particular fungible token from this particular contract
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().contract().eq(&contract) && n.note().symbol().eq(&symbol)).collect();
        }
        vec![]
    }

    pub fn non_fungible_tokens(&self, contract: &Name) -> Vec<NoteEx>
    {
        if contract.raw() == 0
        {
            // select all nfts from all contracts
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().symbol().raw() == 0 && n.note().amount() != 0).collect();
        }
        if contract.raw() != 0
        {
            // select all nfts from this particular contract
            return self.unspent_notes.iter().map(|n| n.clone()).filter(|n| n.note().contract().eq(&contract) && n.note().symbol().raw() == 0 && n.note().amount() != 0).collect();
        }
        vec![]
    }

    pub fn authentication_tokens(&self, contract: &Name, spent: bool) -> Vec<NoteEx>
    {
        if contract.raw() == 0
        {
            // select all auth tokens from all contracts
            return if spent { &self.spent_notes } else { &self.unspent_notes }.iter().map(|n| n.clone()).filter(|n| n.note().symbol().raw() == 0 && n.note().amount() == 0).collect();
        }
        if contract.raw() != 0
        {
            // select all auth tokens from this particular contract
            return if spent { &self.spent_notes } else { &self.unspent_notes }.iter().map(|n| n.clone()).filter(|n| n.note().contract().eq(&contract) && n.note().symbol().raw() == 0 && n.note().amount() == 0).collect()
        }
        vec![]
    }

    pub fn unpublished_notes(&self) -> &HashMap<u64, HashMap<String, Vec<String>>>
    {
        &self.unpublished_notes
    }

    pub fn spending_key(&self) -> Option<SpendingKey>
    {
        if self.is_ivk()
        {
            None
        }
        else
        {
            Some(SpendingKey::from_seed(&self.seed))
        }
    }

    /// Derive a deterministic vault seed at the given index using HMAC-SHA256.
    /// Returns 32 bytes: HMAC-SHA256(wallet_seed, "Cloak_Vault_Seed_v1" || index_le_bytes)
    pub fn derive_vault_seed(&self, index: u32) -> Vec<u8> {
        let mut mac = HmacSha256::new_from_slice(&self.seed)
            .expect("HMAC accepts any key length");
        mac.update(b"Cloak_Vault_Seed_v1");
        mac.update(&index.to_le_bytes());
        mac.finalize().into_bytes().to_vec()
    }

    pub fn default_address(&self) -> Option<Address>
    {
        if self.seed.is_empty()
        {
            // IVK wallet — derive default address from IVK directly
            let (_, d) = self.ivk.dk.find_diversifier(0u64.into())?;
            return self.ivk.to_payment_address(d);
        }
        Some(FullViewingKey::from_spending_key(&SpendingKey::from_seed(&self.seed)).default_address().1)
    }

    pub fn addresses(&self) -> Vec<Address>
    {
        let mut v = vec![];
        if self.seed.is_empty()
        {
            // IVK wallet — derive addresses from IVK directly
            for d in &self.diversifiers
            {
                if let Some((_, diversifier)) = self.ivk.dk.find_diversifier((*d).into()) {
                    if let Some(addr) = self.ivk.to_payment_address(diversifier) {
                        v.push(addr);
                    }
                }
            }
            return v;
        }
        let sk = SpendingKey::from_seed(&self.seed);
        let fvk = FullViewingKey::from_spending_key(&sk);
        for d in &self.diversifiers
        {
            v.push(fvk.find_address((*d).into()).unwrap().1);
        }
        v
    }

    pub fn derive_next_address(&mut self) -> Address
    {
        if self.seed.is_empty()
        {
            // IVK wallet — derive using IVK directly
            let start_di: u64 = self.diversifiers.last().map(|d| *d + 1).unwrap_or(0);
            let (di, d) = self.ivk.dk.find_diversifier(start_di.into())
                .expect("no valid diversifier found");
            let addr = self.ivk.to_payment_address(d)
                .expect("could not derive payment address");
            let di_u64 = u64::try_from(di).unwrap();
            self.diversifiers.push(di_u64);
            return addr;
        }
        let sk = SpendingKey::from_seed(&self.seed);
        let fvk = FullViewingKey::from_spending_key(&sk);
        let mut latest_di = *self.diversifiers.last().unwrap();
        latest_di += 1;
        let (di, addr) = fvk.find_address(latest_di.into()).unwrap();
        latest_di = u64::try_from(di).unwrap();
        self.diversifiers.push(latest_di);
        addr
    }

    pub fn add_leaves(&mut self, leaves: &[u8])
    {
        assert!(leaves.len() % 32 == 0);
        let num_leaves = leaves.len() / 32;
        for i in 0..num_leaves
        {
            let mut bytes = [0; 32];
            bytes.copy_from_slice(&leaves[i*32..(i+1)*32]);
            let leaf = ScalarBytes(bytes);
            self.insert_into_merkle_tree(&leaf);
        }
    }

    fn note_exist_in_unspent(&self, note: &Note) -> bool
    {
        let v: Vec<&NoteEx> = self.unspent_notes.iter().filter(|n| n.note().eq(&note)).collect();
        v.len() > 0
    }

    fn note_exist_in_spent(&self, note: &Note) -> bool
    {
        let v: Vec<&NoteEx> = self.spent_notes.iter().filter(|n| n.note().eq(&note)).collect();
        v.len() > 0
    }

    fn note_exist_in_outgoing(&self, note: &Note) -> bool
    {
        let v: Vec<&NoteEx> = self.outgoing_notes.iter().filter(|n| n.note().eq(&note)).collect();
        v.len() > 0
    }

    // Merkle Tree must be up-to-date before calling this function!
    pub fn add_notes(&mut self, notes: &Vec<String>, block_num: u32, block_ts: u64) -> u64
    {
        // Derive keys based on wallet type:
        // - Full wallet (has seed): derive FVK from spending key → get IVK + OVK
        // - FVK wallet (has fvk_bytes): reconstruct FVK → get IVK + OVK
        // - IVK-only (no seed, no fvk_bytes): use stored IVK directly, no OVK for outgoing recovery
        let (pivk, ovk_opt) = if !self.seed.is_empty() {
            let sk = SpendingKey::from_seed(&self.seed);
            let fvk = FullViewingKey::from_spending_key(&sk);
            (PreparedIncomingViewingKey::new(&fvk.ivk()), Some(fvk.ovk().clone()))
        } else if let Some(fvk) = self.reconstruct_fvk() {
            (PreparedIncomingViewingKey::new(&fvk.ivk()), Some(fvk.ovk().clone()))
        } else {
            (PreparedIncomingViewingKey::new(&self.ivk), None)
        };
        let wallet_ts = SystemTime::now().duration_since(UNIX_EPOCH).expect("Time went backwards").as_millis() as u64;
        let mut fts_received = 0;
        let mut nfts_received = 0;
        let mut ats_received = 0;

        for n in notes
        {
            let encrypted_note = TransmittedNoteCiphertext::from_base64(n);
            if encrypted_note.is_none() { continue; }
            let encrypted_note = encrypted_note.unwrap();

            // test receiver decryption
            match try_note_decryption(&pivk, &encrypted_note) {
                Some(note) => {
                    // verbose: receiver decrypted (uncomment for debugging)
                    // eprintln!("[ADD_NOTES] receiver decrypted: account={:?}, amount={}, symbol={}, is_auth={}, cm={}",
                    //     note.account().to_string(), note.amount(), note.symbol().raw(), note.is_auth_token(),
                    //     hex::encode(note.commitment().to_bytes()));
                    // Merkle tree must be up to date! We check if there's a leaf for this note and get it's array index in the merkle
                    // tree. If there is no index (i.e. leaf), this note can't be valid and is discarded. Only Auth Tokens have no merkle leaves!
                    // Note: Auth tokens use Mint proofs (no nullifiers), so their tree position doesn't matter for spend tracking.
                    let tree_idx = if !note.is_auth_token() {
                        let cm = note.commitment();
                        let cm_bytes = cm.to_bytes();
                        let idx = self.merkle_tree.iter().find_map(|(key, val)| if val.0 == cm_bytes { Some(key) } else { None });
                        if idx.is_none() {
                            log("[ADD_NOTES] WARNING: commitment not found in merkle tree, skipping note");
                            continue;
                        }
                        // verbose: eprintln!("[ADD_NOTES] commitment found in merkle tree at idx={}", idx.unwrap());
                        *idx.unwrap()
                    } else { 0 };
                    let note_ex = NoteEx::from_parts(
                        block_num,
                        block_ts,
                        wallet_ts,
                        tree_idx,
                        note
                    );
                    // make sure a note is not added twice
                    let is_auth = note_ex.note().is_auth_token();
                    if !self.note_exist_in_unspent(note_ex.note())
                    {
                        if self.note_exist_in_spent(note_ex.note())
                        {
                            // Auth tokens with burn=0 are reusable — move back to unspent
                            if is_auth {
                                self.spent_notes.retain(|n| !n.note().eq(note_ex.note()));
                                ats_received += 1;
                                self.unspent_notes.push(note_ex);
                            } else {
                                    // Non-auth spent notes stay spent (normal behavior)
                            }
                        }
                        else
                        {
                            if is_auth {
                                ats_received += 1;
                            }
                            else if note_ex.note().symbol().raw() == 0 && note_ex.note().amount() != 0 { nfts_received += 1; }
                            else { if !note_ex.note().memo().eq(&MEMO_CHANGE_NOTE) { fts_received += 1; } } // don't count 'change' notes
                            self.unspent_notes.push(note_ex);
                        }
                    } else {
                        // already in unspent_notes — skip silently
                    }
                },
                None => {},
            }

            // test sender decryption (only if OVK available — full wallets only, not view-only)
            if let Some(ref ovk) = ovk_opt {
                match try_output_recovery_with_ovk(ovk, &encrypted_note) {
                    Some(note) => {
                        // Auth tokens can be "self-minted" (receiver decrypt succeeds too),
                        // in which case sender recovery would create a duplicate "Sent" entry for the mint.
                        // We only want the incoming mint + (optional) later synthetic burn.
                        if note.is_auth_token() && (self.note_exist_in_unspent(&note) || self.note_exist_in_spent(&note)) {
                            continue;
                        }

                        let note_ex = NoteEx::from_parts(
                            block_num,
                            block_ts,
                            wallet_ts,
                            0,
                            note.clone()
                        );
                        // make sure a note is not added twice
                        if !self.note_exist_in_outgoing(note_ex.note())
                        {
                            self.outgoing_notes.push(note_ex);
                        }
                    },
                    None => {},
                }
            }
        }

        (ats_received << 16) | (nfts_received << 8) | fts_received
    }

    pub fn transaction_history(&self) -> Vec<HistoryTransaction>
    {
        let mut history = vec![];
        let mut received = self.unspent_notes.clone();
        received.append(&mut self.spent_notes.clone());
        received.sort_by(|a, b| b.block_ts().cmp(&a.block_ts()));
        let mut sent = self.outgoing_notes.clone();
        sent.sort_by(|a, b| b.block_ts().cmp(&a.block_ts()));
        // Track timestamps where auth tokens were spent (= authenticate/burn operations)
        let mut burn_timestamps: HashSet<u64> = HashSet::new();
        while !received.is_empty() || !sent.is_empty()
        {
            let mut htx = HistoryTransaction{
                tx_type: "".to_string(),
                date_time: "".to_string(),
                tx_fee: "".to_string(),
                timestamp_ms: 0,
                sort_order: 1,
                account_asset_memo: vec![]
            };
            let mut tx = vec![];

            if !received.is_empty() && !sent.is_empty()
            {
                if sent[0].block_ts() >= received[0].block_ts()
                {
                    tx.push(sent.remove(0));
                    while !sent.is_empty() && sent[0].block_ts() == tx[0].block_ts()
                    {
                        tx.push(sent.remove(0));
                    }
                    htx.tx_type = "Sent".to_string();
                    htx.timestamp_ms = tx[0].block_ts();
                    htx.date_time = DateTime::<Local>::from(UNIX_EPOCH + Duration::from_millis(tx[0].block_ts())).format("%Y-%m-%d %H:%M:%S").to_string();
                }
                else
                {
                    tx.push(received.remove(0));
                    while !received.is_empty() && received[0].block_ts() == tx[0].block_ts()
                    {
                        tx.push(received.remove(0));
                    }
                    htx.tx_type = "Received".to_string();
                    htx.timestamp_ms = tx[0].block_ts();
                    htx.date_time = DateTime::<Local>::from(UNIX_EPOCH + Duration::from_millis(tx[0].block_ts())).format("%Y-%m-%d %H:%M:%S").to_string();
                }

            }
            else if !received.is_empty() && sent.is_empty()
            {
                tx.push(received.remove(0));
                while !received.is_empty() && received[0].block_ts() == tx[0].block_ts()
                {
                    tx.push(received.remove(0));
                }
                htx.tx_type = "Received".to_string();
                htx.timestamp_ms = tx[0].block_ts();
                htx.date_time = DateTime::<Local>::from(UNIX_EPOCH + Duration::from_millis(tx[0].block_ts())).format("%Y-%m-%d %H:%M:%S").to_string();
            }
            else if received.is_empty() && !sent.is_empty()
            {
                tx.push(sent.remove(0));
                while !sent.is_empty() && sent[0].block_ts() == tx[0].block_ts()
                {
                    tx.push(sent.remove(0));
                }
                htx.tx_type = "Sent".to_string();
                htx.timestamp_ms = tx[0].block_ts();
                htx.date_time = DateTime::<Local>::from(UNIX_EPOCH + Duration::from_millis(tx[0].block_ts())).format("%Y-%m-%d %H:%M:%S").to_string();
            }

            // Track total fee and per-action breakdown for this group
            let mut total_fee_raw: u64 = 0;
            let mut fee_symbol: Option<Symbol> = None;
            let mut fee_details: Vec<String> = vec![];

            for n in tx.iter()
            {
                // Special-case (AT UX): if an auth token is minted and burned within the same block timestamp,
                // hide it entirely (suppress both mint and burn). This situation is usually an internal
                // one-shot token and showing it clutters/confuses history.
                if htx.tx_type == "Sent" && n.note().is_auth_token() {
                    let cm = n.note().commitment();
                    if let Some(idx) = received.iter().position(|m|
                        m.block_ts() == n.block_ts() && m.note().is_auth_token() && m.note().commitment() == cm
                    ) {
                        received.remove(idx);
                        continue;
                    }
                }

                // Edge case: suppress outgoing notes that have a matching incoming note
                // (same commitment, same block_ts) to avoid duplicate history entries.
                if htx.tx_type == "Sent" && n.note().account() != Name(0) && !n.note().is_auth_token()
                {
                    let cm = n.note().commitment();
                    if received.iter().any(|m|
                        m.block_ts() == n.block_ts() && m.note().commitment() == cm
                    ) { continue; }
                }
                if !n.note().memo().eq(&MEMO_CHANGE_NOTE)
                {
                    // Skip auth tokens (zero-value ZK commitments) from user-facing history
                    // but record their timestamp so we can label burn operations correctly
                    if n.note().is_auth_token() {
                        if htx.tx_type == "Sent" {
                            burn_timestamps.insert(htx.timestamp_ms);
                        }
                        continue;
                    }
                    // Skip zero-amount notes — internal protocol artifacts (e.g. change=0)
                    if n.note().amount() == 0 { continue; }

                    if n.note().account().eq(&self.alias_authority.actor) && n.note().memo_string().contains("fee")
                    {
                        // Accumulate fee total and track per-action breakdown
                        total_fee_raw += n.note().amount();
                        fee_symbol = Some(n.note().symbol().clone());
                        let memo_str = n.note().memo_string();
                        let detail = format!("{}: {}", memo_str.trim(), n.note().asset().quantity().to_string());
                        fee_details.push(detail);
                    }
                    else if n.note().account().raw() != 0
                    {
                        htx.account_asset_memo.push((
                            n.note().account().to_string(),
                            if n.note().is_auth_token() { hex::encode(n.note().commitment().to_bytes()) } else { n.note().asset().to_string() + &if htx.tx_type.eq("Received") { " (@".to_owned() + &n.note().address().to_bech32m().unwrap() + ")" } else { "".to_string() } },
                            String::from_utf8(n.note().memo()[0..{let len = n.note().memo().iter().position(|&c| c == 0); if len.is_none() {512} else {len.unwrap()}}].to_vec()).unwrap()
                        ));
                    }
                    else
                    {
                        htx.account_asset_memo.push((
                            if htx.tx_type.eq("Sent") { n.note().address().to_bech32m().unwrap() } else { "".to_string() },
                            n.note().asset().to_string() + &if htx.tx_type.eq("Received") { " (@".to_owned() + &n.note().address().to_bech32m().unwrap() + ")" } else { "".to_string() },
                            String::from_utf8(n.note().memo()[0..{let len = n.note().memo().iter().position(|&c| c == 0); if len.is_none() {512} else {len.unwrap()}}].to_vec()).unwrap()
                        ));
                    }
                }
            }
            // Set accumulated fee total
            if total_fee_raw > 0 {
                if let Some(sym) = fee_symbol {
                    htx.tx_fee = Asset::new(total_fee_raw as i64, sym).unwrap().to_string();
                }
            }
            if !htx.account_asset_memo.is_empty() || !htx.tx_fee.is_empty()
            {
                history.push(htx);
            }
            // If this was a fee-only Sent (no account_asset_memo but has fee),
            // save the fee details so we can build a separate fee entry in post-processing
            if total_fee_raw > 0 && history.last().map_or(false, |h| h.tx_type == "Sent") {
                let last = history.len() - 1;
                if history[last].account_asset_memo.is_empty() {
                    // Store fee details as a tag for post-processing
                    history[last].account_asset_memo.push((
                        "Fee".to_string(),
                        "placeholder".to_string(),
                        fee_details.join("\n"),
                    ));
                }
            }
        }

        // Post-processing: label fee-only "Sent" entries with descriptive operation names
        // and set the correct fee amount display.
        for i in 0..history.len() {
            if history[i].tx_type == "Sent"
                && history[i].account_asset_memo.len() == 1
                && history[i].account_asset_memo[0].0 == "Fee"
            {
                let fee_str = history[i].tx_fee.clone();
                let fee_memo = history[i].account_asset_memo[0].2.clone();

                // Determine operation type from same-timestamp entries
                let dt = history[i].date_time.clone();
                let is_vault_op = history.iter().any(|h|
                    h.date_time == dt && h.tx_type == "Received" &&
                    h.account_asset_memo.iter().any(|(a, _, _)| a == "thezeosvault")
                );

                // Check if there's a same-timestamp "Received" that isn't a vault
                // op — indicates a self-send (deduped outgoing note left fee-only group)
                let is_self_send = history.iter().any(|h|
                    h.date_time == dt && h.tx_type == "Received" &&
                    !h.account_asset_memo.iter().any(|(a, _, _)| a == "thezeosvault")
                );

                let label = if is_vault_op {
                    "Vault Withdraw"
                } else if burn_timestamps.contains(&history[i].timestamp_ms) {
                    "Burn Vault"
                } else if is_self_send {
                    "Send Fee"
                } else {
                    "Publish Vault"
                };

                history[i].sort_order = 0; // fee entries sort before main actions
                history[i].account_asset_memo[0] = (
                    label.to_string(),
                    "-".to_owned() + &fee_str,
                    fee_memo,
                );
            }
        }

        // Post-processing: split out fee entries for Vault Deposits.
        // Unlike Vault Withdrawals (where fee notes and received mint are in separate groups),
        // Vault Deposit fee notes are in the same "Sent" group as the deposit amount.
        // The fee is stored in tx_fee but the Dart rendering only reads account_asset_memo,
        // so we must create a separate fee entry.
        let mut deposit_fee_entries = vec![];
        for h in history.iter_mut() {
            if h.tx_type == "Sent" && !h.tx_fee.is_empty()
                && h.account_asset_memo.iter().any(|(a, _, _)| a == "thezeosvault")
            {
                let fee_str = h.tx_fee.clone();
                deposit_fee_entries.push(HistoryTransaction {
                    tx_type: "Sent".to_string(),
                    date_time: h.date_time.clone(),
                    tx_fee: fee_str.clone(),
                    timestamp_ms: h.timestamp_ms,
                    sort_order: 0,
                    account_asset_memo: vec![(
                        "Vault Deposit".to_string(),
                        "-".to_owned() + &fee_str,
                        "".to_string(),
                    )],
                });
                h.tx_fee = "".to_string(); // clear fee from main entry to avoid duplication
            }
        }
        history.append(&mut deposit_fee_entries);

        // Post-processing: split out fee entries for regular sends.
        // Regular sends accumulate fee in tx_fee but don't display it as a
        // separate line item. Create a separate fee entry so it's visible.
        let mut send_fee_entries = vec![];
        for h in history.iter_mut() {
            if h.tx_type == "Sent" && !h.tx_fee.is_empty()
                && h.sort_order == 1  // main entry, not already a fee
                && !h.account_asset_memo.iter().any(|(a, _, _)| a == "thezeosvault")
                && !h.account_asset_memo.iter().any(|(a, _, _)| a == "Fee")
            {
                let fee_str = h.tx_fee.clone();
                send_fee_entries.push(HistoryTransaction {
                    tx_type: "Sent".to_string(),
                    date_time: h.date_time.clone(),
                    tx_fee: fee_str.clone(),
                    timestamp_ms: h.timestamp_ms,
                    sort_order: 0,
                    account_asset_memo: vec![(
                        "Send Fee".to_string(),
                        "-".to_owned() + &fee_str,
                        "".to_string(),
                    )],
                });
                h.tx_fee = "".to_string(); // clear to avoid duplication
            }
        }
        history.append(&mut send_fee_entries);

        // Final sort: newest first by timestamp_ms, then display order within
        // same timestamp: Received (top) → Fee entries (middle) → Sent (bottom).
        // This ensures self-send groups read correctly in a newest-first list.
        history.sort_by(|a, b| {
            // Primary: newest first (descending timestamp)
            let ts_cmp = b.timestamp_ms.cmp(&a.timestamp_ms);
            if ts_cmp != std::cmp::Ordering::Equal { return ts_cmp; }
            // Secondary: display order within same timestamp
            let display_order = |tx: &HistoryTransaction| -> u8 {
                if tx.tx_type == "Received" { 0 }   // top (most recent outcome)
                else if tx.sort_order == 0 { 1 }     // fee entries (middle)
                else { 2 }                            // Sent (bottom, initiated action)
            };
            display_order(a).cmp(&display_order(b))
        });

        history
    }

    /// Estimate the total fee for a send transaction of `send_amount` smallest units.
    /// Simulates note selection (same logic as select_fee_notes in transaction.rs)
    /// to determine how many input notes are needed, then computes the total fee
    /// including per-note spend fees for fragmented wallets.
    /// Returns the fee in smallest units, or None on error.
    pub fn estimate_send_fee(&self, send_amount: u64, fees: &HashMap<Name, Asset>, fee_token_contract: &Name, _recipient_addr: Option<&str>) -> Option<u64>
    {
        // Look up individual fee components
        let begin_fee = fees.get(&Name::from_string(&"begin").ok()?)?.amount() as u64;
        let spendoutput_fee = fees.get(&Name::from_string(&"spendoutput").ok()?)?.amount() as u64;
        let output_fee = fees.get(&Name::from_string(&"output").ok()?)?.amount() as u64;
        let spend_fee = fees.get(&Name::from_string(&"spend").ok()?)?.amount() as u64;

        // Base fee for a standard 1-input send:
        // begin + spendoutput(1) + output(change)
        // Note: publishnotes are auto-embedded in the spend action's note_ct field
        // during zsign, NOT as separate fee-bearing zactions, so no publishnotes fee.
        let base_fee = begin_fee + spendoutput_fee + output_fee;

        // Clone and sort unspent notes (largest first) to simulate selection
        let fee_symbol = fees.values().next()?.symbol().clone();
        let mut notes: Vec<&NoteEx> = self.unspent_notes.iter()
            .filter(|n| {
                fee_token_contract.eq(n.note().contract())
                && fee_symbol.eq(n.note().symbol())
                && !n.note().quantity().is_nft()
                && !n.note().is_auth_token()
            })
            .collect();
        notes.sort_by(|a, b| b.note().amount().cmp(&a.note().amount()));

        // Simulate note selection for the send amount + fee
        // The fee grows dynamically as more notes are selected (same as select_fee_notes)
        let mut accumulated = 0u64;
        let mut total_fee = base_fee;
        let mut notes_selected = 0usize;
        // num_outputs starts at 1 (the change output can be replaced by spendoutput)
        let mut num_outputs = 1usize;

        for note in &notes {
            accumulated += note.note().amount() as u64;
            notes_selected += 1;

            // First note is the primary spendoutput (already in base_fee)
            if notes_selected > 1 {
                if num_outputs > 0 {
                    // Replace an output with spendoutput (cost difference)
                    num_outputs -= 1;
                    total_fee += spendoutput_fee.saturating_sub(output_fee);
                } else {
                    // Pure additional spend
                    total_fee += spend_fee;
                }
            }

            if accumulated >= send_amount + total_fee {
                return Some(total_fee);
            }
        }

        // Not enough notes to cover amount + fee — return best estimate anyway
        // (the UI will show "Insufficient Balance" based on balance check)
        Some(total_fee)
    }

    /// Simulate select_fee_notes from transaction.rs to estimate the total fee
    /// for a transaction that has no existing spend sequence (i.e. the `!found` path
    /// in resolve_ztransaction). This is exact — same logic, same parameters.
    ///
    /// [zaction_fee] — accumulated fee from all zactions (begin + authenticate + mint etc.)
    /// Returns the total fee in smallest units, or None on error.
    fn estimate_fee_payment(&self, zaction_fee: u64, fees: &HashMap<Name, Asset>, fee_token_contract: &Name) -> Option<u64>
    {
        let spendoutput_fee = fees.get(&Name::from_string(&"spendoutput").ok()?)?.amount() as u64;
        let output_fee = fees.get(&Name::from_string(&"output").ok()?)?.amount() as u64;
        let spend_fee = fees.get(&Name::from_string(&"spend").ok()?)?.amount() as u64;

        // Matches resolve_ztransaction's `!found` path exactly:
        //   select_fee_notes(unspent, fees, contract, change=0, fee=zaction_fee+output_fee, num_outputs=1)
        let mut fee = zaction_fee + output_fee;
        let mut num_outputs: usize = 1;

        // Collect and sort available notes (largest first, matching select_fee_notes)
        let fee_symbol = fees.values().next()?.symbol().clone();
        let mut notes: Vec<&NoteEx> = self.unspent_notes.iter()
            .filter(|n| {
                fee_token_contract.eq(n.note().contract())
                && fee_symbol.eq(n.note().symbol())
                && !n.note().quantity().is_nft()
                && !n.note().is_auth_token()
            })
            .collect();
        notes.sort_by(|a, b| b.note().amount().cmp(&a.note().amount()));

        // Exact simulation of select_fee_notes — every note adds structural cost
        let mut sum = 0u64;
        for note in &notes {
            sum += note.note().amount() as u64;

            if num_outputs > 0 {
                num_outputs -= 1;
                fee += spendoutput_fee.saturating_sub(output_fee);
            } else {
                fee += spend_fee;
            }

            if sum >= fee {
                return Some(fee);
            }
        }

        // Not enough notes — return best estimate anyway
        // (UI will compare against balance and show "Insufficient")
        Some(fee)
    }

    /// Estimate burn fee accounting for note fragmentation.
    pub fn estimate_burn_fee(&self, has_assets: bool, fees: &HashMap<Name, Asset>, fee_token_contract: &Name) -> Option<u64>
    {
        let begin_fee = fees.get(&Name::from_string(&"begin").ok()?)?.amount() as u64;
        let authenticate_fee = fees.get(&Name::from_string(&"authenticate").ok()?)?.amount() as u64;
        let mut zaction_fee = begin_fee + authenticate_fee;
        if has_assets {
            let mint_fee = fees.get(&Name::from_string(&"mint").ok()?)?.amount() as u64;
            zaction_fee += mint_fee;
        }
        self.estimate_fee_payment(zaction_fee, fees, fee_token_contract)
    }

    /// Estimate vault creation (auth token publish) fee accounting for note fragmentation.
    pub fn estimate_vault_creation_fee(&self, fees: &HashMap<Name, Asset>, fee_token_contract: &Name) -> Option<u64>
    {
        let begin_fee = fees.get(&Name::from_string(&"begin").ok()?)?.amount() as u64;
        let publishnotes_fee = fees.get(&Name::from_string(&"publishnotes").ok()?)?.amount() as u64;
        let zaction_fee = begin_fee + publishnotes_fee;
        self.estimate_fee_payment(zaction_fee, fees, fee_token_contract)
    }

    // Merkle Tree must be up-to-date before calling this function!
    pub fn digest_block(&mut self, block: &str) -> u64
    {
        let sk = SpendingKey::from_seed(&self.seed);
        let fvk = FullViewingKey::from_spending_key(&sk);
        let j: Value = serde_json::from_str(&block).unwrap();
        let block_num = j["block_num"].as_u64().unwrap() as u32;
        let dt_str = j["timestamp"].as_str().unwrap().to_string() + " +0000"; // need to add (utc) timezone for valid format
        let dt = DateTime::parse_from_str(&dt_str, "%Y-%m-%dT%H:%M:%S%.3f %z").unwrap();
        let block_ts = dt.timestamp_millis() as u64;
        let mut notes_received = 0;
        let mut fts_spent = 0;
        let mut nfts_spent = 0;
        let mut ats_spent = 0;

        // make sure to not sync a block twice
        if block_num <= self.block_num { return 0; }

        for tx in j["transactions"].as_array().unwrap()
        {
            for action in tx["trx"]["transaction"]["actions"].as_array().unwrap()
            {
                if action["account"].as_str().unwrap().eq(&self.alias_authority.actor.to_string())
                {
                    if  action["name"].as_str().unwrap().eq("mint") ||
                        action["name"].as_str().unwrap().eq("spend") ||
                        action["name"].as_str().unwrap().eq("publishnotes")
                    {
                        let mut notes_b64 = vec![];
                        for ct in action["data"]["note_ct"].as_array().unwrap()
                        {
                            notes_b64.push(ct.as_str().unwrap().to_string());
                        }
                        notes_received = self.add_notes(&notes_b64, block_num, block_ts);
                    }
                    if action["name"].as_str().unwrap().eq("spend")
                    {
                        for seq in action["data"]["actions"].as_array().unwrap()
                        {
                            let seq: PlsSpendSequence = serde_json::from_value(seq.clone()).unwrap();

                            for so in seq.spend_output.iter()
                            {
                                // check if published nullifier belongs to one of our notes
                                let index = self.unspent_notes.iter().position(|n| n.note().nullifier(&fvk.nk, n.position()).extract().0.eq(&crate::engine::Scalar::try_from(so.nf.clone()).unwrap()));
                                if index.is_some()
                                {
                                    let note = self.unspent_notes.remove(index.unwrap());
                                    if note.note().symbol().raw() == 0 && note.note().amount() != 0 { nfts_spent += 1; } else { fts_spent += 1; }
                                    self.spent_notes.push(note);
                                }
                            }

                            for s in seq.spend.iter()
                            {
                                // check if published nullifier belongs to one of our notes
                                let index = self.unspent_notes.iter().position(|n| n.note().nullifier(&fvk.nk, n.position()).extract().0.eq(&crate::engine::Scalar::try_from(s.nf.clone()).unwrap()));
                                if index.is_some()
                                {
                                    let note = self.unspent_notes.remove(index.unwrap());
                                    if note.note().symbol().raw() == 0 && note.note().amount() != 0 { nfts_spent += 1; } else { fts_spent += 1; }
                                    self.spent_notes.push(note);
                                }
                            }
                        }
                    }
                    if action["name"].as_str().unwrap().eq("authenticate")
                    {
                        let a: PlsAuthenticate = serde_json::from_value(action["data"]["action"].clone()).unwrap();

                        // check if published note commitment belongs to one of our auth notes
                        let index = self.unspent_notes.iter().position(|n| n.note().commitment().0.eq(&crate::engine::Scalar::try_from(a.cm.clone()).unwrap()));
                        if index.is_some()
                        {
                            if a.burn == 0
                            {
                                // normal access via auth token
                                // TODO: log this somehow?
                            }
                            else
                            {
                                // auth note was burned
                                let burned = self.unspent_notes.remove(index.unwrap());

                                // keep existing behavior: mark it spent
                                self.spent_notes.push(burned.clone());
                                ats_spent += 1;

                                // add a synthetic outgoing entry timestamped at the burn block,
                                // so it appears in history as "Sent" at the time it was burned.
                                let burn_ex = NoteEx::from_parts(
                                    block_num,
                                    block_ts,
                                    block_ts,
                                    0,
                                    burned.note().clone(),
                                );
                                self.outgoing_notes.push(burn_ex);
                            }
                        }
                    }
                }
            }
        }

        // with every block also loop through all unpublished notes and attempt to add every single one of them to this wallet (i.e. try to
        // decrypt them as sender or receiver). This way it is ensured that all unpublished notes related to this wallet are always added
        // automatically which means the user doesn't have to add them manually
        for(_, map) in self.unpublished_notes.clone().into_iter()
        {
            for(_, notes) in map.into_iter()
            {
                notes_received += self.add_notes(&notes, 0, 0);
            }
        }

        self.block_num = block_num;
        (ats_spent << 48) | (nfts_spent << 40) | (fts_spent << 32) | notes_received //(ats_received << 16) | (nfts_received << 8) | fts_received
    }

    pub fn add_unpublished_notes(&mut self, unpublished_notes: &HashMap<String, Vec<String>>)
    {
        let ts = SystemTime::now().duration_since(UNIX_EPOCH).expect("Time went backwards").as_millis() as u64;
        self.unpublished_notes.insert(ts, unpublished_notes.clone());
        // only add the newly inserted notes (not all historical entries)
        for(_, notes) in unpublished_notes.into_iter()
        {
            self.add_notes(notes, 0, 0);
        }
    }

    fn insert_into_merkle_tree(&mut self, leaf: &ScalarBytes) -> u64
    {
        // calculate array index of next free leaf in current tree
        let mut idx = MT_ARR_LEAF_ROW_OFFSET!(MERKLE_TREE_DEPTH) + self.leaf_count % MT_NUM_LEAVES!(MERKLE_TREE_DEPTH);
        // calculate tree offset to translate local array indices of current tree to absolute array indices of global array
        let tos = self.leaf_count / MT_NUM_LEAVES!(MERKLE_TREE_DEPTH) * MT_ARR_FULL_TREE_OFFSET!(MERKLE_TREE_DEPTH);

        // insert this leaf into tree
        self.merkle_tree.insert(tos + idx, leaf.clone());

        // update merkle path
        for d in 0..MERKLE_TREE_DEPTH
        {
            let is_left_child = 1 == idx % 2;
            let sis_idx = if is_left_child { idx + 1 } else { idx - 1 };

            // get values of both sibling nodes and determin which is the right/left one
            //                                                         (parent)                 |                     (parent)
            //                                                         /       \                |                     /       \
            //                                                      (idx)      (0)              |                (sis_idx)   (idx)
            let l = if is_left_child { self.merkle_tree.get(&(tos + idx)).unwrap() }    else { self.merkle_tree.get(&(tos + sis_idx)).unwrap() };
            let r = if is_left_child { &EMPTY_ROOTS[d] }                                else { self.merkle_tree.get(&(tos + idx)).unwrap() };

            // take advantage of the fact that WASM uses little-endian byte encoding
            let mut parent_val: [u8; 32] = Blake2s7rParams::new()
                .hash_length(32)
                .personal(crate::constants::MERKLE_TREE_PERSONALIZATION)
                .to_state()
                .update(&l.0)
                .update(&r.0)
                .finalize()
                .as_bytes()
                .try_into()
                .expect("output length is correct");

            // in case of merkle root: mask the 2 MSBs off the root value, since the arithmetic
            // circuit only supports roots of 254 bits width.
            if d == MERKLE_TREE_DEPTH - 1
            {
                parent_val[31] &= 0x3F;
            }

            // left child's array index divided by two (integer division) equals array index of parent node
            idx = if is_left_child { idx / 2 } else { sis_idx / 2 };

            // check if parent node already exists
            self.merkle_tree.entry(tos + idx).and_modify(|e| *e = ScalarBytes(parent_val)).or_insert(ScalarBytes(parent_val));
        }

        self.leaf_count += 1;
        return tos;
    }

    pub fn get_sister_path_and_root(&self, note: &NoteEx) -> Option<(Vec<Option<([u8; 32], bool)>>, ScalarBytes)>
    {
        let mut idx = note.leaf_idx_arr() % MT_ARR_FULL_TREE_OFFSET!(MERKLE_TREE_DEPTH);
        let tos = note.leaf_idx_arr() / MT_ARR_FULL_TREE_OFFSET!(MERKLE_TREE_DEPTH) * MT_ARR_FULL_TREE_OFFSET!(MERKLE_TREE_DEPTH);

        let mut sis_path = vec![];
        for d in 0..MERKLE_TREE_DEPTH
        {
            let is_left_child = 1 == idx % 2;
            let sis_idx = if is_left_child { idx + 1 } else { idx - 1 };

            let sister = if self.merkle_tree.contains_key(&(tos + sis_idx)) { self.merkle_tree.get(&(tos + sis_idx)).unwrap().clone() } else { EMPTY_ROOTS[d].clone() };
            sis_path.push(Some((sister.0, !is_left_child)));

            // left child's array index divided by two (integer division) equals array index of parent node
            idx = if is_left_child { idx / 2 } else { sis_idx / 2 };
        }

        Some((sis_path, self.merkle_tree.get(&tos).unwrap().clone()))
    }
}

#[cfg(test)]
mod tests
{
    use crate::{
        wallet::Wallet,
        note::{Note, Rseed},
        eosio::{Authorization, Name, Transaction, Action, ExtendedAsset},
        contract::{PlsMint, ScalarBytes, AffineProofBytesLE},
        note_encryption::{NoteEncryption, TransmittedNoteCiphertext, derive_esk, ka_derive_public},
        keys::{SpendingKey, FullViewingKey}
    };
    use rand::rngs::OsRng;

    #[test]
    fn test_serde()
    {
        let mut rng = OsRng.clone();
        let seed = b"this is a sample seed which should be at least 32 bytes long...";
        let fvk = FullViewingKey::from_spending_key(&SpendingKey::from_seed(seed));
        let mut w = Wallet::create(
            seed,
            false,
            [0; 32],
            Name::from_string(&format!("zeos4privacy")).unwrap(),
            Name::from_string(&format!("thezeosvault")).unwrap(),
            Authorization::from_string(&format!("thezeosalias@public")).unwrap(),
            None,
        ).unwrap();

        let notes = vec![
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"7.0000 ZEOS@thezeostoken".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"17.0000 ZEOS@thezeostoken".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"10.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"5.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"4.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"4.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"3.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"3.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"20.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"2.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"2.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"2.0000 EOS@eosio.token".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"12345678987654321@atomicassets".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"99999999998765431@atomicassets".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"88888888887654321@atomicassets".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512]),
            Note::from_parts(0, w.default_address().unwrap(), Name(0), ExtendedAsset::from_string(&"12345677777777321@atomicassets".to_string()).unwrap(), Rseed::new(&mut rng), [0; 512])
        ];

        for n in notes.iter()
        {
            let esk = derive_esk(n).unwrap();
            let epk = ka_derive_public(n, &esk);
            let ne = NoteEncryption::new(Some(fvk.ovk), n.clone());
            let encrypted_note = TransmittedNoteCiphertext {
                epk_bytes: epk.to_bytes().0,
                enc_ciphertext: ne.encrypt_note_plaintext(),
                out_ciphertext: ne.encrypt_outgoing_plaintext(&mut rng),
            };
            w.add_leaves(&n.commitment().to_bytes());
            w.add_notes(&vec![encrypted_note.to_base64()], 0, 0);
        }
        println!("Wallet size: {}", w.size());
        assert!(w.note_exist_in_unspent(w.unspent_notes[0].note()));
        assert!(!w.note_exist_in_unspent(&Note::dummy(&mut rng, None, None).2));

        let mut v = vec![];
        assert!(w.write(&mut v).is_ok());
        assert_eq!(v.len(), w.size());
        let w_de = Wallet::read(&v[..]).unwrap();
        assert!(w == w_de);

        let encoded = w.to_json(false);
        println!("{}", encoded);
        let decoded = Wallet::from_json(&encoded).unwrap();
        assert_eq!(w, decoded);

        println!("{}", serde_json::to_string_pretty(&w.balances()).unwrap());
        println!("{}", serde_json::to_string_pretty(&w.non_fungible_tokens(&Name(0))).unwrap());
    }

    #[test]
    fn test_payload_serde()
    {
        let tx = Transaction{
            actions: vec![
                Action{
                    account: Name(0),
                    name: Name(0),
                    authorization: vec![Authorization{
                        actor: Name(0),
                        permission: Name(0)
                    }],
                    data: serde_json::to_value(PlsMint{
                        cm: ScalarBytes([0; 32]),
                        value: 0,
                        symbol: 0, //Symbol(0),
                        contract: Name(0),
                        proof: AffineProofBytesLE([0; 384])
                    }).unwrap()
                }
            ]
        };

        println!("{}", serde_json::to_string(&tx).unwrap());
    }

    #[test]
    fn test_digest_block()
    {
        //let mut rng = OsRng.clone();

        let blocks = vec![
            r#"{"timestamp":"2024-01-25T00:29:25.000","producer":"eosio","confirmed":0,"previous":"0002b6f5739e885a31d00e6ada3ba0e6d93203260ad96297b9fa09dd6b7d7e3c","transaction_mroot":"1f3664d2f8a6dbcfd2380c53a8fbd8cc72d731ee9bbbc6d48eee8c09b24cfbe7","action_mroot":"8126db3a437e5f9ba0d462e250507c124a3878c0afba8d7deb86df9911098ab5","schedule_version":0,"new_producers":null,"producer_signature":"SIG_K1_JwBxUmkk5RZbdPiHyy5eNNB3NnTuL1fTprtMf669moCSuhAoRuxzcdBpe1pbJ1GdLpiU5fzNDub6d3cSP7kFHKCYKo7uXt","transactions":[{"status":"executed","cpu_usage_us":4219,"net_usage_words":354,"trx":{"id":"9ca5f9d36c727140b31aa59b4ddcf707d81170a44d8d2bc531413da8aa2290e7","signatures":["SIG_K1_KkjbeeVA4FJm3xJR3FZx3qeFTBzg5yqyaif3JRVGCZBV6gf63xnkhEf8GW6KA9ux4y2C6f2ZMRLT4CRvVuuXNqg1G5dPjY","SIG_K1_KigpnoaV1YnbWbUCqxKk7fzXXQZ4ttAesZgMtQc8CRxmdAbd1QTbdzT9oFQVWHRYf1oxLyphkAMZ6JwUTSWk2LdAVKMjU6"],"compression":"zlib","packed_context_free_data":"789c63000000010001","context_free_data":[],"packed_trx":"","transaction":{"expiration":"2024-01-25T00:31:02","ref_block_num":46790,"ref_block_prefix":4244451091,"max_net_usage_words":0,"max_cpu_usage_ms":0,"delay_sec":0,"context_free_actions":[],"actions":[{"account":"thezeosalias","name":"begin","authorization":[{"actor":"thezeosalias","permission":"public"}],"data":"","hex_data":""},{"account":"eosio.token","name":"transfer","authorization":[{"actor":"aliceaccount","permission":"active"}],"data":{"from":"aliceaccount","to":"zeos4privacy","quantity":"15.0000 EOS","memo":"ZEOS transfer & mint"},"hex_data":"90a7a60819855c34e091d9ee5682a9faf04902000000000004454f5300000000145a454f53207472616e736665722026206d696e74"},{"account":"thezeostoken","name":"transfer","authorization":[{"actor":"aliceaccount","permission":"active"}],"data":{"from":"aliceaccount","to":"zeos4privacy","quantity":"25.0000 ZEOS","memo":"ZEOS transfer & mint"},"hex_data":"90a7a60819855c34e091d9ee5682a9fa90d0030000000000045a454f53000000145a454f53207472616e736665722026206d696e74"},{"account":"thezeostoken","name":"transfer","authorization":[{"actor":"aliceaccount","permission":"active"}],"data":{"from":"aliceaccount","to":"thezeosalias","quantity":"7.0000 ZEOS","memo":"tx fee"},"hex_data":"90a7a60819855c34808d8b0653f555cb7011010000000000045a454f5300000006747820666565"},{"account":"thezeosalias","name":"mint","authorization":[{"actor":"thezeosalias","permission":"public"}],"data":{"actions":[{"cm":"90f7e11bd0ced11f9d34e1dbd4a3697a44667af341029d9951ad215a8093923d","value":250000,"symbol":"4,ZEOS","contract":"thezeostoken","proof":""},{"cm":"af83975fffcc611038ee986847b255987cce785438f14dd5d42173353c192c36","value":150000,"symbol":"4,EOS","contract":"eosio.token","proof":""}],"note_ct":["",""]},"hex_data":""},{"account":"thezeosalias","name":"end","authorization":[{"actor":"thezeosalias","permission":"public"}],"data":"","hex_data":""}]}}}],"id":"0002b6f62abb8b161f47f7e9dad878d5034aa95528b410ea0ca60c0a543c2b43","block_num":177910,"ref_block_prefix":3925296927}"#.to_string(),
        ];

        //let leaves = vec![
        //    hex::decode("90f7e11bd0ced11f9d34e1dbd4a3697a44667af341029d9951ad215a8093923d").unwrap(),
        //    hex::decode("af83975fffcc611038ee986847b255987cce785438f14dd5d42173353c192c36").unwrap(),
        //];
        // same data as vec of vec's above but as a single vec
        let leaves = hex::decode("90f7e11bd0ced11f9d34e1dbd4a3697a44667af341029d9951ad215a8093923daf83975fffcc611038ee986847b255987cce785438f14dd5d42173353c192c36").unwrap();

        let mut w = Wallet::create(
            b"army add gossip wrist squeeze chronic simple gold like island wheel north cave praise buddy shine monitor damp into another expose tortoise educate army",
            false,
            [0; 32],
            Name::from_string(&format!("zeos4privacy")).unwrap(),
            Name::from_string(&format!("thezeosvault")).unwrap(),
            Authorization::from_string(&format!("thezeosalias@public")).unwrap(),
            None,
        ).unwrap();

        w.add_leaves(leaves.as_slice());
        for b in blocks
        {
            w.digest_block(&b.to_string());
        }

        println!("{}", w.to_json(true));
        println!("{:?}", serde_json::to_string(&w.get_sister_path_and_root(&w.unspent_notes[0])).unwrap());

    }
}
