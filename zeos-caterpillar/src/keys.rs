...
impl OutgoingViewingKey {
    /// Encodes this Outgoing Viewing Key as Bech32m (ovk1...)
    pub fn to_bech32m(&self) -> Result<String, bech32::Error> {
        bech32::encode("ovk", self.0.to_base32(), Variant::Bech32m)
    }
}
...
impl FullViewingKey {
    ...
    /// Encodes this Full Viewing Key as Bech32m (fvk1...)
    pub fn to_bech32m(&self) -> Result<String, bech32::Error> {
        bech32::encode("fvk", self.to_bytes().to_base32(), Variant::Bech32m)
    }

    /// Parse a Bech32m encoded Full Viewing Key (fvk1...)
    pub fn from_bech32m(s: &str) -> Result<Self, String> {
        let (hrp, data, variant) = bech32::decode(s)
            .map_err(|e| format!("bech32 decode failed: {}", e))?;
        if hrp != "fvk" {
            return Err(format!("unexpected HRP '{}', expected 'fvk'", hrp));
        }
        if variant != bech32::Variant::Bech32m {
            return Err(format!("unexpected bech32 variant {:?}, expected Bech32m", variant));
        }
        let decoded = Vec::<u8>::from_base32(&data)
            .map_err(|e| format!("base32 decoding failed: {}", e))?;
        if decoded.len() != 4 * 32 {
            return Err(format!(
                "from_bech32m: incorrect length, got {} bytes, expected {}",
                decoded.len(), 4 * 32
            ));
        }
        Self::read(&decoded[..]).map_err(|e| format!("from_bech32m: read failed: {}", e))
    }
