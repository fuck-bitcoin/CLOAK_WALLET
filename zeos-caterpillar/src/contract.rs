...
// ============== ABI Serialization Implementations ==============
// These implement EOSIO ABI binary encoding for action data types

use crate::eosio::{AbiSerialize, push_varuint32, push_bytes, push_string, push_name, push_symbol, push_asset, push_uint64, push_uint8};

impl AbiSerialize for ScalarBytes {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.0);
        v
    }
}

impl AbiSerialize for AffineProofBytesLE {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.0);
        v
...
impl AbiSerialize for PlsMint {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.cm.0);
        push_uint64(&mut v, self.value);
        push_uint64(&mut v, self.symbol);
        push_name(&mut v, &self.contract);
        push_bytes(&mut v, &self.proof.0);
        v
    }
}

impl AbiSerialize for PlsMintAction {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_varuint32(&mut v, self.actions.len() as u32);
        for action in &self.actions {
            v.extend(action.abi_serialize());
        }
        push_varuint32(&mut v, self.note_ct.len() as u32);
        for note in &self.note_ct {
            push_string(&mut v, note);
        }
        v
    }
}

impl AbiSerialize for PlsUnshieldedRecipient {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_uint64(&mut v, self.amount);
        push_name(&mut v, &self.account);
        push_string(&mut v, &self.memo);
        v
    }
}

impl AbiSerialize for PlsSpendOutput {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.root.0);
        push_bytes(&mut v, &self.nf.0);
        push_bytes(&mut v, &self.cm_b.0);
        push_bytes(&mut v, &self.cv_net_u.0);
        push_bytes(&mut v, &self.cv_net_v.0);
        push_uint64(&mut v, self.value_c);
        push_symbol(&mut v, &self.symbol);
        push_name(&mut v, &self.contract);
        push_uint8(&mut v, self.cv_eq_gt);
        push_bytes(&mut v, &self.proof.0);
        push_varuint32(&mut v, self.unshielded_outputs.len() as u32);
        for recipient in &self.unshielded_outputs {
            v.extend(recipient.abi_serialize());
        }
        v
    }
}

impl AbiSerialize for PlsSpend {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.root.0);
        push_bytes(&mut v, &self.nf.0);
        push_bytes(&mut v, &self.cv_u.0);
        push_bytes(&mut v, &self.cv_v.0);
        push_bytes(&mut v, &self.proof.0);
        v
    }
}

impl AbiSerialize for PlsOutput {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.cm.0);
        push_bytes(&mut v, &self.cv_u.0);
...
impl AbiSerialize for PlsSpendSequence {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.scm.0);
        push_varuint32(&mut v, self.spend_output.len() as u32);
        for so in &self.spend_output {
            v.extend(so.abi_serialize());
        }
        push_varuint32(&mut v, self.spend.len() as u32);
        for s in &self.spend {
            v.extend(s.abi_serialize());
        }
        push_varuint32(&mut v, self.output.len() as u32);
        for o in &self.output {
            v.extend(o.abi_serialize());
        }
        v
    }
}

impl AbiSerialize for PlsSpendAction {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_varuint32(&mut v, self.actions.len() as u32);
        for action in &self.actions {
...
impl AbiSerialize for PlsAuthenticate {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_bytes(&mut v, &self.cm.0);
        push_name(&mut v, &self.contract);
        ...
        for pa in &self.actions {
            push_name(&mut v, &pa.account);
            push_name(&mut v, &pa.name);
            push_varuint32(&mut v, pa.authorization.len() as u32);
            for auth in &pa.authorization {
                push_name(&mut v, &auth.actor);
                push_name(&mut v, &auth.permission);
            }
            push_bytes(&mut v, &pa.data);
        }
        push_uint8(&mut v, self.burn);
        push_bytes(&mut v, &self.proof.0);
        v
    }
}

impl AbiSerialize for PlsAuthenticateAction {
    fn abi_serialize(&self) -> Vec<u8> {
        self.action.abi_serialize()
    }
}

impl AbiSerialize for PlsPublishNotesAction {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_varuint32(&mut v, self.note_ct.len() as u32);
        for note in &self.note_ct {
...
impl AbiSerialize for PlsWithdraw {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_name(&mut v, &self.contract);
        push_asset(&mut v, &self.quantity);
        push_string(&mut v, &self.memo);
        push_name(&mut v, &self.to);
        v
    }
}

impl AbiSerialize for PlsWithdrawAction {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_varuint32(&mut v, self.actions.len() as u32);
        for action in &self.actions {
...
impl AbiSerialize for PlsFtTransfer {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_name(&mut v, &self.from);
        push_name(&mut v, &self.to);
        push_asset(&mut v, &self.quantity);
        push_string(&mut v, &self.memo);
        v
...
impl AbiSerialize for PlsNftTransfer {
    fn abi_serialize(&self) -> Vec<u8> {
        let mut v = Vec::new();
        push_name(&mut v, &self.from);
        push_name(&mut v, &self.to);
        push_varuint32(&mut v, self.asset_ids.len() as u32);
        for asset in &self.asset_ids {
            push_asset(&mut v, asset);
        }
        push_string(&mut v, &self.memo);
        v
    }
